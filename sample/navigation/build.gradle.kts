...
dependencies {
  ...
  // that file must compile — so `GezginTestNavigator` has to be visible to `compileKotlin`.
  // `compileOnly`
  // keeps this test-only artifact OUT of the app's runtime classpath (the accessors are only ever
  // called
  // from the `test` source set); `testImplementation` below re-adds it for the test
  // compile+runtime.
...
// F-MAJOR-2 — the headline UI-less test API (`GezginTestNavigator.fromX()`) now works in the
// canonical
// multi-module layout (graphs in `main`, tests in `test`). The earlier `kspTest`-scoped attempt was
// structurally wrong: `TestApiCodegen` needs the [GraphModel] built from scanning `@NavGraph`
// symbols in
// THIS KSP round's SOURCES, but a `kspTestKotlin` round is handed only the `test` source set's
// `.kt` files —
// `AppNav.kt`'s graphs live in `main` (already compiled to .class), and KSP's
// `getSymbolsWithAnnotation`
// never re-discovers annotations off binary classpath entries → empty model → nothing emitted. The
// fix is
// to run the accessor codegen in the round that OWNS the graphs: set the flag on the GLOBAL `ksp
// {}`
// extension (below) so it rides `kspKotlin` (the `main` round, alongside topology/navigator
// codegen), which
// emits `GezginTestAccessors.kt` into `main`; the `test` source set then sees the compiled
// accessors and
