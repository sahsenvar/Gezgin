...
/**
 * F-MAJOR-2 — the headline UI-less test flow, exercised in the canonical multi-module layout:
 * graphs live in `main`, these tests in `test`. `gezgin.emitTestAccessors=true` (set on the
 * module's MAIN KSP round — see `build.gradle.kts`) makes the processor emit typed
 * `GezginTestNavigator.fromX()` accessors into `main`, so this `test` source set drives navigation
 * through them directly (`nav.fromLogin().goToSignUp()`), with no `raw.xNavigator(entryId)` factory
 * plumbing. The class-level `@GezginInternalApi` opt-in survives only for the handful of low-level
 * `raw.navigate` / `raw.currentEntryId` setup / inspection calls a couple of tests still make — the
 * `fromX()` accessors themselves need no opt-in.
 */
@OptIn(ExperimentalCoroutinesApi::class, GezginInternalApi::class)
class AppNavBehaviorTest {

  @Test
  fun loginSignUpQuitAndGoToWelcome_leavesLoginThenWelcome() {
  ...
  @Test
  fun welcomeContinueToDashboard_replacesWelcomeKeepsLoginBelow() {
    val nav = GezginTestNavigator(start = AuthGraph.LoginScreenRoute, topology = gezginTopology)
    nav.fromLogin().goToSignUp()
    nav.fromCredentials().goToProfileInfo("ada@example.com")
    nav.fromProfileInfo().goToTerms()
  ...
  // Login, then replaceTo always pushes fresh → a STACKED second Login, not a dedup. Left as
  // authored.
  // This pins the RAW-PUSH topology semantics and deliberately does NOT model the real app login
  // path
  // (loginSuccess = @ReplaceTo(Dashboard) consumes the seed Login — see the sibling test below).
  @Test
  fun logoutFromRawPushedStack_stacksASecondLoginEntry() {
    val nav = GezginTestNavigator(start = AuthGraph.LoginScreenRoute, topology = gezginTopology)
    ...
    assertEquals(
      listOf(
        AuthGraph.LoginScreenRoute,
        HomeGraph.DashboardScreenRoute,
        ProfileGraph.ProfileScreenRoute,
        ProfileGraph.SettingsScreenRoute,
      ),
      nav.backStack,
    )
  ...
  // Real app login path: loginSuccess = @ReplaceTo(Dashboard) (AuthGraph.kt) CONSUMES the seed
  // Login, so
  // after logout = @ReplaceTo(Login, clearUpTo=Dashboard, inclusive) the stack holds a SINGLE Login
  // — the
  // on-device result (checklist item 14), unlike the raw-push sibling above (which never consumes
  // the seed).
  @Test
  fun logoutAfterRealLoginSuccess_leavesSingleLoginEntry() {
    val nav = GezginTestNavigator(start = AuthGraph.LoginScreenRoute, topology = gezginTopology)
  ...
  @Test
  fun avatarFlowQuitWith_deliversValueToProfilePickAvatarResults() = runTest {
    val nav =
      GezginTestNavigator(start = ProfileGraph.ProfileScreenRoute, topology = gezginTopology)
  ...
  @Test
  fun nestedZoomFlowBack_popsOnlyZoomLeavesCropOnTop() {
    val nav =
      GezginTestNavigator(start = ProfileGraph.ProfileScreenRoute, topology = gezginTopology)
    ...
    assertEquals(
      listOf(
        ProfileGraph.ProfileScreenRoute,
        AvatarFlow.PickSourceScreenRoute,
        AvatarFlow.CropScreenRoute("camera"),
      ),
      nav.backStack,
    )
  }
  ...
  @Test
  fun nestedZoomFlowQuitWith_deliversValueToProfileTearsDownWholeAvatarFlow() = runTest {
    val nav =
      GezginTestNavigator(start = ProfileGraph.ProfileScreenRoute, topology = gezginTopology)
    val profile = nav.fromProfile()
    ...
    assertEquals(NavResult.Value(AvatarChoice("zoomed://frame")), profile.pickAvatarResults.first())
  }

  @Test
  fun goToRelatedTwiceSameId_createsThreeDistinctStackEntries() {
  ...
  @Test
  fun forgotPasswordSuspendResult_deliversValue() = runTest {
    val nav = GezginTestNavigator(start = AuthGraph.LoginScreenRoute, topology = gezginTopology)
  ...
  @Test
  fun forgotPasswordBack_deliversCanceled() = runTest {
    val nav = GezginTestNavigator(start = AuthGraph.LoginScreenRoute, topology = gezginTopology)
    val login = nav.fromLogin()
  ...
  @Test
  fun termsBackToStart_landsOnCredentialsKeepsLoginBelow() {
    val nav = GezginTestNavigator(start = AuthGraph.LoginScreenRoute, topology = gezginTopology)
    nav.fromLogin().goToSignUp()
    nav.fromCredentials().goToProfileInfo("ada@example.com")
    nav.fromProfileInfo().goToTerms()
  ...
  @Test
  fun termsQuit_tearsDownSignUpFlowLeavesLoginOnTop() {
    val nav = GezginTestNavigator(start = AuthGraph.LoginScreenRoute, topology = gezginTopology)
    nav.fromLogin().goToSignUp()
    nav.fromCredentials().goToProfileInfo("ada@example.com")
    nav.fromProfileInfo().goToTerms()
  ...
  @Test
  fun backToDashboardThenCrossGraphGoToProfile_bothViaGeneratedNavigators() {
    val nav = GezginTestNavigator(start = HomeGraph.DashboardScreenRoute, topology = gezginTopology)
  ...
  @Test
  fun itemImageViewerBackToItemDetail_popsOnlyModalLeavesDashboardAndItemDetail() {
    val nav = GezginTestNavigator(start = HomeGraph.DashboardScreenRoute, topology = gezginTopology)
  ...
  @Test
  fun goToHelpThenBackToDashboard_roundTripsViaGeneratedNavigators() {
    val nav = GezginTestNavigator(start = HomeGraph.DashboardScreenRoute, topology = gezginTopology)
  ...
  @Test
  fun pickSortSuspendResult_deliversSortOrder() = runTest {
    val nav = GezginTestNavigator(start = HomeGraph.DashboardScreenRoute, topology = gezginTopology)
  ...
  @Test
  fun pickSortLaunchStream_deliversValueToPickSortResults() = runTest {
    val nav = GezginTestNavigator(start = HomeGraph.DashboardScreenRoute, topology = gezginTopology)
    val dashboard = nav.fromDashboard()
  ...
  @Test
  fun notificationsSheetSuspendResult_deliversSelectedLevel() = runTest {
    val nav =
      GezginTestNavigator(start = ProfileGraph.ProfileScreenRoute, topology = gezginTopology)
    val profile = nav.fromProfile()
  ...
  @Test
  fun notificationsSheetBack_deliversCanceled() = runTest {
    val nav =
      GezginTestNavigator(start = ProfileGraph.ProfileScreenRoute, topology = gezginTopology)
    val profile = nav.fromProfile()
