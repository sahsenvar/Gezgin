...
class MainActivity : ComponentActivity() {
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    // DEBUG-only PD-corruption harness (maestro madde-7): only on a debuggable build launched with
    // the
    // `corrupt_state` extra. When false, `base` stays null → content runs directly = identical to
    // prod.
    // Passed on the ORIGINAL launch intent so it survives process death: a launcher resume replays
    // the
    // task's original intent (a fresh `am start` extra would be dropped), keeping the hook armed on
    // restore.
    val corrupt =
...
private fun ShoprApp(onRootBack: () -> Unit) {
  ...
  GezginDisplay(navigator = navigator) { shopGraphEntries() }
}
