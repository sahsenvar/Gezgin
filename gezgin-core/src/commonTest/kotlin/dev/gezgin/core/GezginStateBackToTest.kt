...
class GezginStateBackToTest {
  @Test
  fun backToBottomInclusiveKeepsBottom_neverEmpties() {
  ...
  @Test
  fun popsToNearestAncestorExclusive() {
    val s = GezginState(emptyList(), 0, testTopology)
  ...
  @Test
  fun missingTargetReturnsNull_noMutation() {
    val s = GezginState(emptyList(), 0, testTopology)
    s.push(Feed)
  ...
  @Test
  fun inclusiveTrueAlsoRemovesTarget() {
    val s = GezginState(emptyList(), 0, testTopology)
    s.push(Product("A"), singleTop = false)
    s.push(Feed)
    s.push(Product("B"), singleTop = false)
    s.push(Catalog)
    val removed =
      s.backTo(Product::class, inclusive = true)!! // nearest = Product("B"), also remove it
    assertEquals(listOf(Product("B"), Catalog), removed.map { it.route })
    assertEquals(Feed, s.stack.last().route)
  }

  @Test
  fun nearestAncestorAmongThreeSameTypeEntries_inclusive() {
    val s = GezginState(emptyList(), 0, testTopology)
    s.push(Product("A"), singleTop = false)
    s.push(Product("B"), singleTop = false)
    s.push(Feed)
    s.push(Product("C"), singleTop = false)
    s.push(Catalog)
  ...
  @Test
  fun targetIsTopOnlyReturnsNull() {
    val s = GezginState(emptyList(), 0, testTopology)
    s.push(Catalog)
    assertNull(s.backTo(Catalog::class, inclusive = false))
