...
class GezginStateReplaceTest {
    ...
    @Test fun popRemovesAndReturnsTop() {
        val s = GezginState(emptyList(), 0, testTopology)
        s.push(Feed); s.push(Catalog); val top = s.push(Product("1"))!!
        assertEquals(top, s.pop())
        assertEquals(listOf<Route>(Feed, Catalog), s.stack.map { it.route })
    }
    @Test fun replaceUpToAncestorExclusiveKeepsAncestor() {
        val s = GezginState(emptyList(), 0, testTopology)
        s.push(Feed); s.push(Catalog); s.push(Product("1"))
    ...
    @Test fun replaceUpToMissingClearUpToThrows_noMutation() {
        val s = GezginState(emptyList(), 0, testTopology)
        s.push(Feed)
        assertFailsWith<IllegalArgumentException> {
            s.replaceUpTo(Product("2"), clearUpTo = Catalog::class, inclusive = true)
        }
        assertEquals(listOf<Route>(Feed), s.stack.map { it.route })   // require temizlikten ÖNCE → mutasyon yok
    }
    @Test fun replaceUpToUsesNearestAncestorNotFirst() {
        val s = GezginState(emptyList(), 0, testTopology)
