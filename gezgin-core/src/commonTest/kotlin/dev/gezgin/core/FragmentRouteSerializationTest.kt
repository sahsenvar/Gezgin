 ...
 * `dev.gezgin.core.fragment.Route.toBundle()` / `decodeGezginRoute()` (Task 6.0 §2c/§2-decision: the genuinely
 * risky half needs no `Bundle` and no Robolectric; the Bundle glue is Android-only and compile-verified).
 *
 * `toBundle`/`decodeGezginRoute` encode a `Route` **polymorphically** via `PolymorphicSerializer(Route::class)`
 * using the app's `Json` (the SAME instance backing Gezgin's backstack PD — reused, no second Json). This
 * mirrors that exact mechanism against a test `SerializersModule` (like [SavedStateTest]) to prove: any route
 * that survives Gezgin PD survives `toBundle()` by construction. A round-trip through the polymorphic
 * serializer IS what happens inside `Bundle.putString(KEY, encode(route))` → `getString(KEY)` → `decode(...)`.
 */
class FragmentRouteSerializationTest {
    ...
    /** Encode → decode a route through the polymorphic serializer, exactly as toBundle/decodeGezginRoute do. */
    ...
    fun encodingIsPolymorphic_carriesTypeDiscriminatorSoDecodeCanPickTheSubtype() {
        // The encoded String must carry the polymorphic type tag (that's what lets a Fragment's argsless
