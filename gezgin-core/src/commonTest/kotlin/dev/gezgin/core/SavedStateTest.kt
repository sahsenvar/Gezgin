...
class SavedStateTest {
  ...
  private val missingResultSerializerTopology =
    GezginTopology(
      flowChains = emptyMap(),
      flowStarts = emptyMap(),
  ...
  @Test
  fun pendingDeliveredResultSurvivesProcessDeath() =
  ...
  @Test
  fun nextIdSurvives_noIdCollisionAfterRestore() =
    kotlinx.coroutines.test.runTest {
      val n1 = RawNavigator(Feed, testTopology)
      n1.navigate(Catalog)
      val saved = n1.save()
      val n2 = RawNavigator(Feed, testTopology, onRootBack = {}, json = Json, restored = saved)
      n2.navigate(Product("z"))
      assertEquals(n2.keys.map { it.id }.toSet().size, n2.keys.size) // benzersiz id'ler
    }

  // --- Additional required coverage: in-flight slot round-trip, late collector on restored
  // navigator ---

  @Test
  fun inFlightSlotRoundTrips_lateBackWithResultDeliversAfterRestore() = runTest {
    val n1 = RawNavigator(Feed, testTopology)
    n1.navigate(Catalog)
    ...
    val saved =
      json.decodeFromString(
        ...
        json.encodeToString(SavedState.serializer(), n1.save()),
      )
    ...
    val r =
      n2.results<OrderId>("Catalog→CheckoutFlow").first() // late re-attach after restore + delivery
  ...
  @Test
  fun canceledSlotRoundTrips_lateCollectorReceivesCanceled() = runTest {
    val n1 = RawNavigator(Feed, testTopology)
    n1.navigate(Catalog)
    n1.launchForResult("Catalog→CheckoutFlow", Cart)
  ...
  // --- Review fix: encode/decode symmetry — açık polimorfik payload restore'da ctor json'ıyla
  // decode edilir ---

  @Test
  fun polymorphicResultSurvivesRestore_withModuleAwareJson() = runTest {
    val n1 = RawNavigator(Feed, testTopology, onRootBack = {}, json = json)
  ...
  @Test
  fun saveFailsWhenDeliveredResultSlotHasNoSerializer() {
  ...
  @Test
  fun restoreFailsWhenValuePayloadHasNoSerializer() {
    val saved =
      SavedState(
    ...
    val error =
      assertFailsWith<IllegalArgumentException> {
        RawNavigator(
          Feed,
          missingResultSerializerTopology,
          onRootBack = {},
          json = json,
          restored = saved,
        )
  ...
  // adopt edilmeye çalışılınca decodeSlot IllegalArgumentException fırlatır AMA navigator'ın
  // stack'i
  // DOKUNULMADAN `start`'ta kalır. Bu atomiklik, Android adopt yolunun
  // (RememberNavigator.android.kt)
  // istisnayı yutup graceful fresh-start yapmasını mümkün kılar — desktop ile simetri. Eski
  // (mutasyon-
  ...
  @Test
  fun adoptRestoredIsAtomic_badSlotLeavesNavigatorAtStart() {
    val nav = RawNavigator(Feed, missingResultSerializerTopology)
    ...
    assertEquals(
      Feed,
      nav.current,
      "adopt fırlarsa navigator start'ta kalmalı (atomik, yarı-adopt yok)",
    )
