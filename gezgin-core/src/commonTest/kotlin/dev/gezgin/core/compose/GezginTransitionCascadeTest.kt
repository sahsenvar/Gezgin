...
/**
 * Task 3.5 gate — pure-Kotlin (Compose kurulumu yok) cascade çözümü testleri (§9: "Cascade en
 * içteki (screen) > graph > app"). Screen>graph basamağı [dev.gezgin.core.Route.transition]'ın
 * interface override zincirinden geldiği için burada AYRICA sınanmıyor gibi görünebilir — ama
 * [resolveTransition] tam olarak `route.transition` okuyarak bu zinciri tetikliyor, dolayısıyla ilk
 * iki senaryo o zincirin DOĞRU çalıştığının kanıtı. §9 per-call override (N1) API'de hiç yok
 * (yalnız `raw.navigate` escape-hatch'i — Task 3.5 kapsamında negatif test gerekmiyor, ayrı bir
 * "transition" parametresi/override noktası tanımlanmadı; bkz. task-3.5-report.md).
 */
