...
/**
 * Task 3.5 fix (review) — ayırt edici per-entry metadata testleri: transition cascade artık
 * `NavDisplay` parametrelerinden DEĞİL, entry'nin KENDİ `NavEntry.metadata`'sından iner (pop B→A'da
 * B'nin `backward{}` spec'inin kullanılabilmesi için — top-route yaklaşımında A'nınki okunuyordu,
 * §9 ihlali).
 *
 * Anahtar assertion'ları decompile'daki string sabitlerine DEĞİL, Nav3'ün PUBLIC sarmalayıcılarının
 * ürettiği gerçek anahtarlara (`NavDisplay.popTransitionSpec { null }.keys` vb.) karşı yapılır —
 * anahtarın iç temsili platforma göre farklı (alpha05: String sabiti, android 1.1.4:
 * `NavMetadataKey.toString()`), sarmalayıcı-üretimi anahtar her platformda kendi NavDisplay'iyle
 * tutarlı.
 */
class GezginEntryMetadataTest {
  ...
  fun `back-only route'un entry metadata'sinda popTransitionSpec anahtari VAR, forward YOK`() {
    val entry =
      scope()
        .toNavEntry(
          GezginKey(route = ScreenBackOnlyTransition, id = 2L),
          navigator,
          navTransitions {},
        )
    assertTrue(
      popKey in entry.metadata,
      "pop anahtari bekleniyordu; actual: ${entry.metadata.keys}",
    )
    assertTrue(
      forwardKey !in entry.metadata,
      "forward anahtari OLMAMALI; actual: ${entry.metadata.keys}",
    )
  }
  ...
  fun `transition'siz route'un (app default'u da yokken) entry metadata'si BOS`() {
    ...
    assertTrue(
      ...
      "bos metadata bekleniyordu; actual: ${entry.metadata.keys}",
    )
  }
  ...
  fun `forward-only route'ta forward anahtari VAR, pop ve predictive YOK (null alan anahtar eklemez)`() {
    ...
    assertTrue(
      forwardKey in entry.metadata,
      "forward anahtari bekleniyordu; actual: ${entry.metadata.keys}",
    )
    assertTrue(popKey !in entry.metadata, "pop anahtari OLMAMALI; actual: ${entry.metadata.keys}")
    assertTrue(
      predictiveKey !in entry.metadata,
      "predictive anahtari OLMAMALI (back da yok); actual: ${entry.metadata.keys}",
    )
  }
  ...
  fun `transition'siz route app default'u varsa ONUN metadata'sini tasir (cascade app basamagi)`() {
    val entry =
      scope()
        .toNavEntry(
          GezginKey(route = Feed, id = 1L),
          navigator,
          navTransitions { forward { error("spec cagirilmadan sadece anahtar kontrolu") } },
        )
    assertTrue(
      forwardKey in entry.metadata,
      "app-default forward anahtari bekleniyordu; actual: ${entry.metadata.keys}",
    )
  }
  ...
  fun `bottom sheet metadata yalniz sheetGesturesEnabled degisince esit degildir`() {
    fun props(gesturesEnabled: Boolean): GezginBottomSheetProps {
      ...
      val entry =
        scope().toNavEntry(GezginKey(route = route, id = 10L), navigator, navTransitions {})
