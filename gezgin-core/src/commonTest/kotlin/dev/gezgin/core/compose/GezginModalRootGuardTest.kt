...
/**
 * Faz4 final-review Important 1 (§7) — modal-kind-at-root DINAMIK guard'ının saf-JVM adapter-level
 * pini (Compose runtime GEREKMEZ). [toNavEntry] `isRoot=true` iken kind SCREEN DIŞINDAysa
 * (DIALOG/BOTTOM_SHEET/ FULLSCREEN_MODAL) `require` fırlatır — bu ANA guard TÜM dinamik yolları
 * (replaceTo/quitAndGoTo ile modal'ı köke koyma) kapatır. SCREEN root geçer. Nav3 OverlayScene
 * invariant'ı: modal genuinely kökte tek başına var olamaz (altında ≥1 SCREEN entry şart).
 */
class GezginModalRootGuardTest {
  ...
  @Test fun `DIALOG kok entry require firlatir`() = assertRootModalRejected(DialogDefault("x"), 1L)

  @Test
  fun `BOTTOM_SHEET kok entry require firlatir`() = assertRootModalRejected(SheetDefault("x"), 2L)

  @Test fun `FULLSCREEN_MODAL kok entry require firlatir`() = assertRootModalRejected(FullModal, 3L)

  @Test
  fun `SCREEN kok entry GECER`() {
    // Fırlatmamalı — SCREEN kök meşru.
    scope()
      .toNavEntry(GezginKey(route = Feed, id = 4L), navigator, navTransitions {}, isRoot = true)
  }
  ...
  fun `modal entry kok DEGILSE (isRoot=false) GECER`() {
    ...
    scope()
      .toNavEntry(
        GezginKey(route = DialogDefault("x"), id = 5L),
        navigator,
        navTransitions {},
        isRoot = false,
      )
