 ...
 * KİMLİĞİ stabil kalmalı: VM ctor'unda yakalanan referans, restore'dan sonra da display'in gözlemlediği
 * state'i sürmeli.
 *
 ...
 * çekirdeği [RawNavigator.adoptRestored] doğrudan pinlenir (aynı instance state'i re-point eder, yeni
 * instance ÜRETMEZ) + eski çift-instance restore tasarımının VM↔display referanslarını AYRIŞTIRDIĞI
 * (C1'in ta kendisi) karakterize edilir.
 */
class NavigatorIdentityRestoreTest {
    ...
    fun `adoptRestored AYNI instance state'i re-point eder - VM referansi restore edilen state'i surer`() {
        ...
        // VM-referansı üzerinden navigasyon → display'in gözlemlediği kaynakta (aynı flow) anında görünür
    ...
    fun `karakterizasyon - eski cift-instance restore VM ve display referanslarini AYRISTIRIR (C1 bug)`() {
        // HEAD davranışı: `rememberSaveable` restore'da YENİ bir RawNavigator kurardı (decodeNavigatorState).
        ...
        val displayObserved = decodeNavigatorState(encoded, start = Feed, topology = testTopology, json = testJson, onRootBack = {})
    ...
    fun `decodeSavedStateOrNull gecerli snapshot'i decode eder, bozuk-bos null doner`() {
        ...
        assertNull(decodeSavedStateOrNull("{ not valid json", testJson))         // bozuk json → null (fresh-start)
