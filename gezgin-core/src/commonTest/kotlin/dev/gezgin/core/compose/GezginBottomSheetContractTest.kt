...
/**
 * Task 4.2 (§7) — adapter'ın BottomSheet contract okuması + guard'ının saf pini (Compose runtime
 * GEREKMEZ): [toNavEntry] `NavEntry.metadata`'sına [GEZGIN_BOTTOM_SHEET_KEY] altında yazdığı
 * [GezginBottomSheetProps]'un route'un opsiyonel [dev.gezgin.core.BottomSheetContract]'ından
 * (runtime değer, §2.4) DOĞRU geldiğini doğrular. `@NoBack + dismissOnBackPress` guard'ı DIALOG'la
 * ORTAK yardımcıdan geçer (aynı davranış).
 */
class GezginBottomSheetContractTest {
  ...
  fun `dismissOnClickOutside - contract'tan iner (Important 1 - scrim-tap knob acikta)`() {
    ...
    assertTrue(
      sheetPropsOf(SheetDefault("x"), 32L).dismissOnClickOutside,
      "contract'sız → default true",
    )
  }
  ...
  fun `sheetGesturesEnabled default true ve route getter false metadata'ya iner`() {
    ...
    assertTrue(
      sheetPropsOf(SheetDefault("x"), 33L).sheetGesturesEnabled,
      "contract'sız default true",
    )
    assertTrue(
      !sheetPropsOf(SheetCustom("x"), 34L).sheetGesturesEnabled,
      "getter false metadata'ya inmeli",
    )
  }
  ...
  fun `drag handle default korunur ve getter None metadata'ya iner`() {
    ...
    assertEquals(
      ...
      sheetPropsOf(SheetDefault("x"), 35L).dragHandleMode,
    )
    assertEquals(
  ...
  fun `guard - @NoBack + default BottomSheetContract require firlatir`() {
    val ex =
      assertFailsWith<IllegalArgumentException> {
        scope()
          .toNavEntry(
            GezginKey(route = SheetBackDismissable, id = 5L),
            navigator,
            navTransitions {},
          )
      }
