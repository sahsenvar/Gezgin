 ...
 *
 * Kurulum: `register<R>(noBack = ...)` content'i burada hiç invoke edilmez (boş @Composable lambda),
 * yalnız registry flag'i okunur.
 */
class GezginOnBackTest {
    ...
    fun `kok muafiyeti - tek entry noBack olsa bile onBack onRootBack tetikler (app'e hapsolmaz)`() {
        ...
        val nav = RawNavigator(start = Product("x"), topology = testTopology, onRootBack = { rootBackCount++ })
        val scope = GezginEntryScope().apply {
            register<Product>(noBack = true) { }
        }
        assertEquals(1, nav.backStack.value.size)
