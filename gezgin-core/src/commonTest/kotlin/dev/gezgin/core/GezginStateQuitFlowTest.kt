...
class GezginStateQuitFlowTest {
  @Test
  fun quitPopsOnlyOwnInstance_evenWhenContiguous() {
    val s = GezginState(emptyList(), 0, testTopology)
    s.push(Feed)
    s.push(Cart, enterFlow = true)
    s.push(Payment) // instance 1
    s.push(Cart, enterFlow = true, singleTop = false)
    s.push(Payment, singleTop = false) // instance 2 (bitişik!)
  ...
  @Test
  fun quitOnRootFlowReturnsNull_stackIntact() {
    val s = GezginState(emptyList(), 0, testTopology)
    s.push(Cart, enterFlow = true)
    s.push(Payment) // flow = root
    assertNull(s.quitFlow(s.currentFlowId()!!))
    assertEquals(2, s.stack.size)
  }

  @Test
  fun nestedFlowQuitOnlyQuitsInnerFlow() {
    val s = GezginState(emptyList(), 0, testTopology)
    s.push(Feed)
    s.push(Cart, enterFlow = true)
    s.push(Payment) // checkoutFlow instance
  ...
  @Test
  fun quitOuterFlowFromNestedPopsAll() {
    val s = GezginState(emptyList(), 0, testTopology)
    s.push(Feed)
    s.push(Cart, enterFlow = true)
    s.push(Payment) // checkoutFlow instance 1
    val outerFlowId = s.currentFlowId()!!
  ...
  @Test
  fun quitUnknownFlowIdReturnsNull_noMutation() {
    val s = GezginState(emptyList(), 0, testTopology)
    s.push(Feed)
    s.push(Cart, enterFlow = true)
    s.push(Payment)
  ...
  @Test
  fun quitFlowWhenTopIsOutsideFlow_returnsNull_noMutation() {
    val s = GezginState(emptyList(), 0, testTopology)
    s.push(Feed)
