 ...
 *
 * Task 3.2 devri: `toNavEntry(key)` → `toNavEntry(key, navigator)` (top-entry-drive, additive) — bu
 * testlerde navigator yalnız parametre olarak geçiyor, hiçbir assertion navigator davranışına
...
class GezginEntryScopeTest {
  ...
  fun `register - cift kayit aciklayici hata firlatir`() {
    ...
    val error = assertFailsWith<IllegalStateException> { scope.register<Feed> {} }
    assertTrue(
  ...
  fun `toNavEntry - kayitsiz route icin lookup aninda hata firlatir (content invoke edilmeden)`() {
    ...
    val error =
      assertFailsWith<IllegalStateException> { scope.toNavEntry(key, navigator, navTransitions {}) }
    assertTrue(
  ...
  fun `toNavEntry - farkli id'ler ayni route icin farkli contentKey uretir (aynı-route iki instance ayrimi)`() {
    ...
    val first =
      scope.toNavEntry(GezginKey(route = Product("a"), id = 1L), navigator, navTransitions {})
    val second =
      scope.toNavEntry(GezginKey(route = Product("a"), id = 2L), navigator, navTransitions {})
