 ...
 *    onu BİR KEZ [RawNavigator.adoptRestored] ile benimser. Adopt tetikleyicisi [NavigatorHolder.adoptChecked]
 *    bayrağıdır: holder'a bağlı, TAZE kurulduğu composition'da `false` → adopt (snapshot varsa) → `true`.
 *    Config-change'te holder (dolayısıyla bayrak) retained → rotasyonda re-adopt YOK (MN-1: bayrak snapshot
 *    boş olsa DA ilk composition'da set edilir; canlı navigator zaten doğru state'i taşıdığından snapshot'la
 *    üzerine yazmak hem gereksiz hem de teslim edilmiş bir slotu diriltme (çift-teslim) hazard'ı taşırdı).
 *    PD'de taze holder → bayrak `false` → adopt tam bir kez. `save` daima CANLI navigator'dan encode eder
 *    (held değer yok sayılır) → snapshot güncel. Bozuk/şema-uyumsuz snapshot [decodeSavedStateOrNull] ile
 *    `null` → sessiz fresh-start.
 *
 * **Çoklu-navigator (MJ-A):** holder `viewModel<NavigatorHolder>(key = …)` ile call-site-stabil bir key
 * altında edinilir. Key `rememberSaveable`'da tutulan benzersiz bir token'dır; iki bağımsız `rememberNavigator`
 * çağrısı (master/detail, adaptif iki-pane, overlay navigator) AYRI rememberSaveable slot'una (composition
 * konumundan türetilir) düşer → AYRI token → AYRI holder. Token config-change'te Bundle'dan aynen restore
 * edilir (retained holder'ı geri bulur), PD'de de korunur → determinist-stabil. Key'SİZ `viewModel<T>()`
 * sınıf-adı tabanlı SABİT default key kullanırdı → aynı Activity'de (aynı `ViewModelStoreOwner`) iki
 * navigator TEK holder'a sessizce çakışır, ikinci çağrı birincinin instance'ını (yanlış start/topology)
 * alırdı. Token, base'in `rememberSaveable{RawNavigator}` call-site ayrımının birebir eşleniğidir.
 */
@Composable
 ...
 * çıkar; [adoptChecked] snapshot-adopt'un holder ömrü başına EN FAZLA BİR KEZ — yalnız holder TAZE kurulduğu
 * (PD ya da ilk açılış) composition'da — çalışmasını garanti eder (MN-1: config-change'te re-adopt olmaz).
 */
