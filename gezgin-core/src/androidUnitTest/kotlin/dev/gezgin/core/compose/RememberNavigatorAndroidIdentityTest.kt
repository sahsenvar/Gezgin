...
class RememberNavigatorAndroidIdentityTest {
    ...
    fun `retained owner rejects colliding changed namespace token while same namespace keeps holder`() {
        ...
        try {
            val initial = composeNavigator(activity, owner, restoredValues = null, restoreKey = "Aa", start = Feed)
            initial.navigator.navigate(Catalog)

            val recreationState = initial.registry.performSave()
            initial.view.disposeComposition()
            val sameKey = composeNavigator(activity, owner, recreationState, restoreKey = "Aa", start = Feed)
    ...
    fun `saved state registry export imports same namespace snapshot into fresh activity owner`() {
        ...
        val restoredController = launchRegistryActivity(
    ...
    fun `saved state registry rejects Aa snapshot for fresh BB activity despite composite hash collision`() {
        ...
        val changedController = launchRegistryActivity(
            restoreKey = "BB",
            start = START_PRODUCT,
            restoredState = exportedState,
        )
    ...
    fun `same business key keeps simultaneous call sites distinct across registry recreation`() {
        val initialController = launchRegistryActivity(
        ...
        assertNotSame(initialLeft, initialRight, "distinct call-site tokens must resolve distinct holders")
        ...
        try {
            ...
            assertNotSame(restoredLeft, restoredRight, "restored call-site tokens must resolve distinct holders")
    ...
    private fun launchRegistryActivity(
        ...
        val intent = Intent(RuntimeEnvironment.getApplication(), NavigatorSavedStateActivity::class.java)
...
internal class NavigatorSavedStateActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            val left = rememberNavigator(
                start = start,
                topology = testTopology,
                json = androidTestJson,
                restoreKey = restoreKey,
            )
            navigators = if (navigatorCount == 1) {
                listOf(left)
            } else {
                val right = rememberNavigator(
                    start = Product("right"),
                    topology = testTopology,
                    json = androidTestJson,
                    restoreKey = restoreKey,
