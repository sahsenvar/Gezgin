 ...
 * `rememberSaveable` içeriği [GezginDisplay]'in wire ettiği
 * `rememberSaveableStateHolderNavEntryDecorator` sayesinde entry başına (contentKey =
 * `GezginKey.id`) ayrı slot'ta yaşar. Bu JB CMP 1.11.0 desktop portu `assertExists` yok → "yok"
 * kontrolleri `onAllNodesWithText(...).assertCountEquals(0)`.
 *
 ...
 * kapsamı saf-fonksiyon [encodeNavigatorState]/[decodeNavigatorState] üzerinden
 * RememberNavigatorSaverTest'te (task 3.2 deliverable e) korunur.
 */
@OptIn(ExperimentalTestApi::class)
class GezginDisplayR2Test {

  /**
   * Sayaç ekranı — `rememberSaveable`; tıklama artırır. Tag ile hedeflenir (aynı anda yalnız top
   * compose edilir).
   */
  @Composable
  ...
  fun `R2 - ayni-degerli iki Detail entry AYRI saved state alir (id bazli, deger bazli degil)`() =
    runComposeUiTest {
      ...
      // (2) Top #b TAZE (0), #a'nın 1'inden BAĞIMSIZ → id-başına, değer-başına DEĞİL. #b'yi 2'ye
      // getir.
      ...
      // decorator'ının (deliverable 1) kablolandığının davranışsal kanıtı: decorator olmadan #a
      // taze
      // (0) compose edilir ve bu "Detail=1" assertion'ı RED olur (bkz. task-3.3-report.md TDD
      // kanıtı).
      onNodeWithText("Detail=1").assertIsDisplayed()
  ...
  @Test
  fun `4a - replaceTo ayni-deger-farkli-id yeni contentKey render eder ve saved state SIFIRLANIR`() =
    runComposeUiTest {
      val nav = RawNavigator(start = Product("42"), topology = testTopology)
      setContent {
        GezginDisplay(navigator = nav) {
      ...
      // Aynı DEĞER, YENİ id ile replace — backStack (Route listesi) DEĞİŞMEZ; keysState (id)
      // değişir →
      // recompose + yeni contentKey → taze saveable slot (Detail=0). keysState olmadan (backStack
      // dedup)
      // bu recompose HİÇ tetiklenmez ve ekran Detail=1'de kalırdı (4a red kanıtı,
      // task-3.3-report.md).
