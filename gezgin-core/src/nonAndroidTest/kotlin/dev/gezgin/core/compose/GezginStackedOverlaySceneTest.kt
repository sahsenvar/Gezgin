 ...
 * o alt-liste yeniden DialogScene(B) `overlaidEntries=[Screen]`, en dip SinglePane(Screen). Her
 * modal KENDİ overlay window'unda compose edilir → hepsi aynı anda ağaçta; en üstteki (C) aktif.
 * `back()` tek tek soyar (C → B → Screen). Karışık stack (dialog + sheet) da aynı peel döngüsünden
 * geçer — DialogSceneStrategy ve GezginBottomSheetSceneStrategy top-entry metadata'sına göre ayrık
 * seçilir.
 *
 * **uiTest'te pinlenen:** her katmanın compose'da var olması (`assertExists` — overlay semantiği:
 * alttaki soyulmaz), en üstteki aktifin görünürlüğü, `back()`'in tam bir katmanı kapatıp bir
 * alttakini ortaya çıkarması, stack sırasının `nav.current`'a yansıması. **on-device'a bırakılan:**
 * N-derin scrim katmanının görsel z-order'ı/opaklığı ve predictive-back animasyonu (window-katmanı
 * görsel doğrulaması headless uiTest'te güvenilir okunamaz — §7 "on-device doğrulanacak" + 4.4
 * checklist).
 */
