 ...
 *
 * Decorator'sız (eski `emptyList()`) actual'a karşı bu test KIRMIZI'dır: ya `viewModel()` ilk
 * render'da `LocalViewModelStoreOwner` yokluğundan patlar, ya da tüm entry'ler pencere-scoped TEK
 * store'u paylaşıp `assertNotSame` (a) düşer. Fix'ten sonra üç davranış da geçer:
...
class DesktopViewModelStoreDecoratorTest {
  ...
  private fun ProbeScreen(onResolved: (ProbeVm) -> Unit) {
    ...
    SideEffect {
      onResolved(vm)
    } // her başarılı composition'da o an görünür entry'nin VM'ini bildir
    BasicText(
      text = "probe=$bump",
  ...
  fun `desktop entry-scoped VM - iki instance ayri VM, pop clear eder, cover-recompose korur`() =
    runComposeUiTest {
      ...
      waitForIdle()
      val vmA =
        requireNotNull(topVm) { "start entry VM çözülmeliydi (owner yoksa burada patlardı)" }

      // (c) aynı entry'yi recompose et → viewModel() AYNI instance döner (VM recomposition'da
      // yaşar).
      ...
      // (b) top'u pop et → #b'nin store'u clear → vmB.onCleared; vmA hâlâ canlı (cover'dan
      // kurtuldu).
      nav.back()
      waitForIdle()
