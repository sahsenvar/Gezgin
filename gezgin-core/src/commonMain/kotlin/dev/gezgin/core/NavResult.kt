...
/**
 * The result returned from a result-producing destination (`ResultRoute<T>`/`ResultFlow<T>`, §6). The
 * suspend `goToXForResult()` or the `xResults` stream yields this; the consumer matches on [Value]/[Canceled].
 */
