...
public sealed interface NavEvent {
   ...
   * happened (graceful no-op, mirroring [BackToTargetMissing]). Emitted e.g. when a `@ReplaceTo`
   * edge is fired twice in quick succession (a user double-tap): the first call already removed
   * `clearUpTo`, so the second finds it absent. The alternative — throwing from the `clearUpTo`
   * lookup — would crash the app on an ordinary double-tap, so this is an observable event instead.
   *
   * @property target the missing target route name
   */
