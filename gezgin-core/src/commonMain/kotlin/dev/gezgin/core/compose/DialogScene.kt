...
/**
 * [GezginDialogScene]'in `NavEntry.metadata`'da taşındığı anahtar (Gezgin-tanımlı, iki platformda AYNI
 * string). Nav3'ün built-in `DialogSceneStrategy.DialogKey`'inin YERİNE kullanılır: built-in DialogScene
 * dismiss'i `SceneStrategyScope.onBack` (= tekil `NavDisplay.onBack`) üzerinden çalıştırır ve bu scope'un
 * onBack'i modül-dışından enjekte EDİLEMEZ (internal ctor) → dismiss'i sahip-entry'ye pinleyemez. Bu yüzden
 * Gezgin, [GezginBottomSheetScene] gibi, kendi DialogScene'ini yazar ve dismiss'i entry-pinli
 * `back(entryId)`'e bağlar (C-MJ-1). Sheet key'inden ve transition key'lerinden ayrık.
 */
 ...
 * `overlaidEntries` = alttaki entry'ler (arka SCREEN görünür kalır). `equals`/`hashCode` Nav3 `Scene`
 * sözleşmesi için elle (built-in DialogScene deseni; `onBack` karşılaştırılmaz — lambda kimliği kararsız).
 */
internal class GezginDialogScene(
    ...
    init {
        // Overlay scene invariant (§7) — toNavEntry'nin modal-kind-at-root guard'ı bunu zaten önceler;
 ...
 * `DialogSceneStrategy`'nin yerini alır ([GezginNavDisplay] actual'larında).
 */
internal class GezginDialogSceneStrategy(private val onDismiss: (Long) -> Unit) : SceneStrategy<Route> {
