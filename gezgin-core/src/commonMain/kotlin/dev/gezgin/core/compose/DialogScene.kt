 ...
 * yazar ve dismiss'i entry-pinli `back(entryId)`'e bağlar (C-MJ-1). Sheet key'inden ve transition
 * key'lerinden ayrık.
 */
internal const val GEZGIN_DIALOG_KEY = "gezginDialog"

/**
 * Gezgin-sahipli Dialog `OverlayScene` (§7, C-MJ-1) — Nav3'ün built-in `DialogScene`'inin birebir
 * işlevsel kopyası, TEK farkla: `onDismissRequest` sahip-entry'ye pinlenmiş [onBack]'e bağlanır
 * (built-in tekil `NavDisplay.onBack`'e bağlar → çifte-dismiss / geç-dismiss ALTTAKİ ekranı
 * poplardı). [onBack] burada `{ navigator.back(entry.contentKey) }` ile beslenir → modal artık top
 * değilken dismiss NO-OP olur. `overlaidEntries` = alttaki entry'ler (arka SCREEN görünür kalır).
 ...
 * (adapter [toNavEntry] `kind == DIALOG`/`FULLSCREEN_MODAL` iken yazar) varsa bir
 * [GezginDialogScene] döndürür, yoksa `null` (zincirdeki sonraki stratejiye devret). Dismiss,
 * [onDismiss] ile sahip-entry id'sine (`lastEntry.contentKey`) pinlenir → modal artık top değilken
 * pop no-op olur. Built-in `DialogSceneStrategy`'nin yerini alır ([GezginNavDisplay]
 * actual'larında).
 */
