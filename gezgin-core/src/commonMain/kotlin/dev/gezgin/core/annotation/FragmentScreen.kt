...
/**
 * Brownfield Fragment interop (§11/§11.1/§11.2) — marks a legacy Android `androidx.fragment.app.Fragment`
 * as a **leaf destination** in Gezgin's Nav3-based navigation, registered alongside the function-target
 * kinds (`@Screen`/`@Dialog`/`@BottomSheet`/`@FullscreenModal`). Unlike those, this annotation stands on
 * the **Fragment class itself** (CLASS target), because the Fragment — not a composable — is the content:
 *
 * ```
