...
/**
 * Brownfield Fragment interop (§11/§11.1/§11.2) — marks a legacy Android
 * `androidx.fragment.app.Fragment` as a **leaf destination** in Gezgin's Nav3-based navigation,
 * registered alongside the function-target kinds
 ...
 * deliberately **no dialog/bottom-sheet Fragment variant** (§11.2 — Fragment interop is
 * screen-only; no `@FragmentDialog`/`@FragmentBottomSheet`). No special multiplatform handling is
 * needed at the declaration level — the annotation simply never resolves a target on non-Android
 * sources.
 *
