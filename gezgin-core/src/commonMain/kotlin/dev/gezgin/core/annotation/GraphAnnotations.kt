...
/**
 * Transparent graph container: marks a sealed interface that groups routes; its members are
...
/**
 * Opaque flow container: marks a sealed interface whose members form a single flow-unit;
...
/**
 * Marks the start destination inside a `@NavGraph`/`@FlowGraph` — the first route opened on
