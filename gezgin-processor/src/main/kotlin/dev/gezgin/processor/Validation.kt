 ...
 * IDE tooling can key off it; KSP errors fail the enclosing compilation, which is the enforcement
 * mechanism.
 *
...
internal class GezginValidator(private val model: GraphModel, private val logger: KSPLogger) {
  ...
  /**
   * [NavigatorCodegen]'in HER navigator sınıfında koşulsuz taşıdığı üye adları — bkz.
   * [checkN10Members]. `raw`/`entryId` her navigator'da SABİT property olarak taşınır
   * (NavigatorCodegen.kt:152-155); `back` (`@NoBack` hariç) da fiilen koşulsuz.
   * `quit`/`quitWith`/`backWithResult` koşullu ama hiçbir edge'in TÜRETMEDİĞİ ad'lar olduğundan
   * rezerve tutulur (yalnız `name=` override'ı çakışabilir). NOT: `backToStart` KASITEN burada
   * DEĞİL — `@BackTo(routeNamedStart)` onu meşru şekilde tek üye olarak türetebildiğinden rezerve
   * etmek yanlış-pozitif olur; çakışması [checkN10Members]'daki `size >= 2` (BACK_TO_START kaydı)
   * mekanizmasıyla — yalnız gerçekten iki `backToStart()` varsa — yakalanır.
   */
  private val RESERVED_MEMBER_NAMES =
    setOf("back", "quit", "quitWith", "backWithResult", "raw", "entryId")
   ...
   *   may. A source already inside the target flow is exempt: entry rules constrain crossing the
   *   boundary, not movement within it.
   *
  ...
  private fun checkE1(route: RouteModel) {
    route.edges
      .filter {
        it.kind == EdgeKind.GO_TO ||
          it.kind == EdgeKind.REPLACE_TO ||
          it.kind == EdgeKind.QUIT_AND_GO_TO
      }
      .forEach { edge ->
        val target = edge.targetFq
        val targetAsGraph = graphsByFq[target]
        val parent = parentGraphOf(target)
        val entersResultFlowDirectly =
          targetAsGraph != null &&
            targetAsGraph.declaresResultFlowDirectly &&
            targetAsGraph.fqName !in route.flowChainFq
        val entersResultFlowViaStart =
          parent != null &&
            parent.declaresResultFlowDirectly &&
            parent.startFq == target &&
   ...
   * ONE exemption — the legal container-entry level. A route target exempts its INNERMOST enclosing
   * flow when it IS that flow's `@StartDestination` (entering a flow via its start is precisely
   * what a `@GoTo(SomeFlow::class)` container edge pushes); a flow-container target exempts itself
   * (its own ancestors still count). Everything ABOVE the exempted level must already be in the
   * source's chain — the walk covers the whole ancestor chain, closing the grandchild-start hole:
   * from outside AvatarFlow, `@GoTo(ZoomRoute)` (start of the nested ZoomFlow) exempts ZoomFlow but
   * still crosses AvatarFlow's boundary → E3 (the old single-level parent check let it slip past
   * both E3 and E1, silently bypassing AvatarFlow's result contract). From INSIDE AvatarFlow the
   * same edge stays legal (AvatarFlow is in the source's chain — "içeride serbest @GoTo").
   *
   * Back-edges (`@BackTo`) are intentionally out of scope here: forward edges define topology, back
        ...
        val crossedFlows: List<String> =
          when {
            targetRoute != null -> {
              ...
              if (innermost != null && graphsByFq[innermost]?.startFq == target) chain.dropLast(1)
              else chain
            }
  ...
  /**
   * Flows lexically enclosing graph [graphFq] (outermost→innermost), NOT including [graphFq]
   * itself.
   */
  ...
  // region Graph and flow parent structure
   ...
   * `OrderGraph : AppGraph` (one parent) is fine; declaring two annotated parents is not. The
   * graph-level parallel of the route-level E5.
   */
  ...
  /**
   * An öksüz (orphan) `@FlowGraph`: lexically NESTED inside a non-graph declaration yet declaring
   * no annotated graph/flow supertype, so neither subtyping (`: ParentGraph`) nor nesting connects
   * it to a graph and its members float free. A TOP-LEVEL `@FlowGraph` is a legitimate
   * root/independent flow (entered by an edge, e.g. `@GoForResult`) — [GraphModelNode.isNested] is
   * false there, so it is exempt.
   */
  ...
  /**
   * Every `@NavGraph`/`@FlowGraph` must be a `sealed interface`. Cross-file member discovery
   * ([dev.gezgin.processor.model.ModelReader]) uses `getSealedSubclasses`, which per the KSP
   ...
   * misleading SD1/E6 or pure silence. A lexically-nested member survives via the fallback, hiding
   * the gap; enforcing `sealed interface` universally makes the derivation sound. A non-interface
   * graph is rejected too (an `object`/`class` can't be the `: G` supertype its routes need) — not
   * covered by any E-rule.
   */
  ...
  // region Forward-edge and @BackTo target resolution
   ...
   * it). A `@NavGraph` target has no start to navigate to, and a model-external/unresolved class is
   * not navigable at all — both would make codegen crash (`getValue`/`requireNotNull`) with an
  ...
  // region Generated navigator name collisions

  /**
   * Two sources whose stripped simple name collapses to the same `X` both emit an `XNavigator` into
   * the single generated package — the second silently overwrites the first. Only routes that
  ...
  /**
   * Within a single source, two edges whose derived method names coincide (e.g. `@GoTo(Detail)` and
   * `@GoTo(DetailRoute)` both → `goToDetail`, or two `@GoForResult`s sharing a `name=` → the same
   * `launchX`/`xResults`/`goToXForResult` triple) would emit duplicate members. Each `@GoForResult`
   * records all three triple members (not just `launchX`) so a collision against either sibling is
   * caught too.
   *
  ...
  private fun checkN10Members(route: RouteModel) {
    ...
    byMember.keys
      .filter { it in RESERVED_MEMBER_NAMES }
      .forEach { member ->
        error(
          "N10",
   ...
   * either defaulted or nullable (codegen passes `null` to nullable params without defaults when
  ...
  // region Same-kind duplicate edge disambiguation
  ...
  private fun checkFX1(route: RouteModel) {
    val hasFlowScopedBackEdge =
      route.backEdges.any { it.kind == BackEdgeKind.BACK_TO_START || it.kind == BackEdgeKind.QUIT }
  ...
  /**
   * `@QuitAndGoTo` inside a ResultFlow silently drops the expected result — use a result-returning
   * edge instead.
   */
  ...
  // region Deliberately omitted modal guards
  //
  // A `@QuitAndGoTo(modal)` warning is NOT reliably implementable in
  ...
  private fun parentGraphOf(fq: String): GraphModelNode? =
    model.graphs.firstOrNull { fq in it.memberFq }

  /**
   * The `@StartDestination` of [route]'s innermost enclosing flow — the target `@BackToStart`
   * returns to.
   */
