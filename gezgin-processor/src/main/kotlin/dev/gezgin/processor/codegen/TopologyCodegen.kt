...
internal object TopologyCodegen {
  ...
  /**
   * Shortest common package prefix across every graph/route fqName in [model]. Empty if [model] is
   * empty.
   */
  ...
  private fun commonDotPrefix(a: String, b: String): String =
    a.split(".").zip(b.split(".")).takeWhile { (x, y) -> x == y }.joinToString(".") { it.first }
  ...
  fun generateTopology(model: GraphModel, packageName: String): FileSpec {
    ...
    val property =
      PropertySpec.builder("gezginTopology", GEZGIN_TOPOLOGY).initializer(topologyInit).build()

    return FileSpec.builder(packageName, GENERATED_TOPOLOGY_FILE)
      // The topology initializer calls @GezginInternalApi-gated GezginTopology, FlowType, and
  ...
  /**
   * `GezginRememberNavigator.kt`: emits ONLY the stable `gezginJson`
   * (`Json(gezginSerializersModule)`), a top-level `val` (one instance per process — the
   * encode/decode symmetry the PD-restore Saver needs) so app call sites stop hand-assembling
   * `remember { Json { … } }`. Emitted alongside `GezginSerializers.kt` (same `emitSerializers`
   ...
   *
   * NO `@Composable` is generated here: the graph module is plain-JVM in the canonical layout (no
   * Compose compiler plugin), so a `@Composable` FUNCTION compiled here would lack Compose lowering
   * and crash Compose consumers at runtime with `NoSuchMethodError` (see the body
  ...
  fun generateRememberNavigator(packageName: String): FileSpec {
    ...
    return FileSpec.builder(packageName, GENERATED_REMEMBER_FILE).addProperty(jsonProp).build()
  }
  ...
  fun generateSerializers(model: GraphModel, packageName: String): FileSpec {
    ...
    val property =
      PropertySpec.builder("gezginSerializersModule", SERIALIZERS_MODULE)
        .initializer(moduleInit)
        .build()

    return FileSpec.builder(packageName, GENERATED_SERIALIZERS_FILE).addProperty(property).build()
  }
