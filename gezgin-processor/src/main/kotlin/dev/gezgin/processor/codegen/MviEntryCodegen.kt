...
internal object MviEntryCodegen {
  ...
  private fun provideMviEntryFun(entry: EntryFunctionModel): FunSpec {
    ...
    val vmLambdaType =
      LambdaTypeName.get(parameters = lambdaParams, returnType = vmClass)
        .copy(annotations = listOf(AnnotationSpec.builder(COMPOSABLE).build()))
    val viewModelParam =
      ParameterSpec.builder("viewModel", vmLambdaType)
        .apply {
          if (emitDefault)
            defaultValue(defaultResolver(vm, vmClass, vmHasNav, vmHasRoute, ::roleOf))
        }
        .build()
    // endregion
  ...
  /**
   * The DI-detected default `{ [nav, ]args -> … }` resolver lambda (only reached when
   * `emitDefault`).
   */
      ...
      VmDiKind.ANDROIDX -> {
        ...
        val ctorArgs =
          vm.ctorParams
            ...
            .joinToString(", ") {
              "${it.name} = ${if (roleOf(it) == VmDiClassifier.Role.NAV) "nav" else "args"}"
            }
        CodeBlock.of(
          "{ %L -> %M(factory = %M { %M { %T(%L) } }) }",
          header,
  ...
  private fun registerBody(
    ...
    if (mvi.bottomBar != null) {
      body.add(
        "val imeVisible = %T.%M.getBottom(%T.current) > 0\n",
        WINDOW_INSETS,
        IME,
        LOCAL_DENSITY,
      )
