...
/**
 * Reads every `@FragmentScreen(Route::class)`-annotated CLASS (the current contract brownfield
 ...
 *   change via a PUBLIC no-arg ctor (`clazz.getConstructor().newInstance()`). `FS1` rejects every
 *   shape that would pass the frontend but crash inside `FragmentFactory.instantiate`: an
 ...
 *   kind annotation's now (`@Screen(Route::class)` is likewise rejected, as `SC9`) — so
 *   `@FragmentScreen(Route::class)` would emit a `register<Route>` no concrete-class push ever
 *   matches — a DEAD registration. → no model.
 * - **`FS7` — screen-only, no modal contract ().** A route implementing a modal presentation
 ...
 * `@Screen`, whose generated `XScreen(route, nav)` call makes a wrong route/navigator type a
 * COMPILE error, a Fragment reads its route/nav via hand-written `private val args by
 * gezginArgs<R>()` / `private val nav by gezginNav<N>()` delegates whose `R`/`N` are NOT tied to
 * `@FragmentScreen(route)`. A mismatch (`gezginArgs<WrongRoute>()` after a copy-paste) compiles —
 * both are `Route`/navigator types — and fails only at RUNTIME as a `ClassCastException` in
 * gezgin-core's `gezginBoundRoute(...) as R` / `gezginBoundNav(...) as N`. This
 * weaker-than-`@Screen` type-safety is NOT closed here because it is not reliably tractable in KSP:
 * KSP is a DECLARATION-level API — it exposes property/function declarations and their resolved
 * types, but NOT delegate/initializer EXPRESSIONS, function BODIES, or the explicit type arguments
 * of arbitrary call sites. So the processor cannot attribute a property to a
 * `gezginArgs`/`gezginNav` call (there is no delegate-expression API — it sees only that `args` has
 * some inferred type), and it cannot see the equally-valid INLINE usages (`gezginArgs<R>()` called
 * inside `onViewCreated`/any body — invisible to KSP). A heuristic ("find a `Route`-typed property,
...
internal class FragmentModelReader(
  ...
  fun read(): Pair<List<FragmentEntryModel>, Boolean> {
    val models =
      resolver
        .getSymbolsWithAnnotation(FRAGMENT_SCREEN_FQ)
