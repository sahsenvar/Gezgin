...
internal class ModelReader(private val resolver: Resolver, private val logger: KSPLogger) {
  ...
  fun read(): GraphModel {
    ...
    val memberDecls =
      graphDecls
        .flatMap { graph ->
          graph.getSealedSubclasses() + graph.declarations.filterIsInstance<KSClassDeclaration>()
        }
  ...
  private fun collectGraphDeclarations(): List<KSClassDeclaration> {
    val navGraphs =
      resolver.getSymbolsWithAnnotation(NAV_GRAPH_FQ).filterIsInstance<KSClassDeclaration>()
    val flowGraphs =
      resolver.getSymbolsWithAnnotation(FLOW_GRAPH_FQ).filterIsInstance<KSClassDeclaration>()
  ...
  private fun buildGraphNode(
    graphDecl: KSClassDeclaration,
    memberDecls: List<KSClassDeclaration>,
  ): GraphModelNode {
    ...
    decl.annotations
      .flatMap { annotation ->
        ...
        if (targetTypes.isEmpty()) {
          error(
            "@${annotation.shortName.asString()} on ${decl.requireQualifiedName()} has no resolvable target"
          )
        }
