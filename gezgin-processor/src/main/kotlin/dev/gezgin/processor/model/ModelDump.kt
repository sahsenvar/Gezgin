...
internal fun GraphModel.dumpText(): String {
  ...
  graphs
    ...
    .forEach { graph ->
      lines +=
        "graph ${graph.fqName} flow=${graph.isFlow} resultFlow=${graph.isResultFlow} " +
  ...
  sortedRoutes.forEach { route ->
    lines +=
      "route ${route.fqName} graph=${route.graphFq} chain=${route.flowChainFq.joinOrDash()} " +
  ...
  sortedRoutes.forEach { route ->
    route.edges.forEach { edge ->
      lines +=
        "edge ${route.fqName} ${edge.kind} ${edge.targetFq} singleTop=${edge.singleTop} " +
  ...
  sortedRoutes.forEach { route ->
    route.backEdges.forEach { backEdge ->
      lines +=
        "backedge ${route.fqName} ${backEdge.kind} target=${backEdge.targetFq.orDash()} " +
