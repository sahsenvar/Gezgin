 ...
 * - [HILT_PLAIN] — `@HiltViewModel` with NO assisted factory → `hiltViewModel<VM>()`; Gezgin
 *   supplies NOTHING and Nav3 has no path that writes the route into `SavedStateHandle`, so this VM
 *   receives NO route data (a parameterized route is rejected with `MV12`, `EntryModelReader`). It
 *   always has a default resolver and never wires nav; use [HILT_ASSISTED] for a
 *   route-data-carrying screen.
 * - [KOIN] — `@KoinViewModel`, `@InjectedParam` ctor params → `koinViewModel { parametersOf(args,
 *   nav) }`.
 * - [ANDROIDX] — no DI annotation at all → `viewModel(factory = viewModelFactory { initializer {
 *   VM(...) } })` (the shape proven to compile in `gezgin-mvi`'s `CounterMvi` fixture); every ctor
 *   param is Gezgin-supplied, so the default is emitted only when they are all `route`/`nav`-typed.
 */
internal enum class VmDiKind {
  HILT_ASSISTED,
  HILT_PLAIN,
  KOIN,
  ANDROIDX,
 ...
 * The `route`/`nav` types are RECONSTRUCTED from the entry (`args: XRoute`, `nav: XNavigator`), and
 * an "other" param only ever forces the `viewModel` param to become required — its type is never
 * printed. Capturing only [typeFq] also sidesteps `toTypeName()` throwing on a same-module `nav:
 * XNavigator` whose generated navigator type is not yet resolvable in the KSP round that reads it
 * (nav is matched by [name] `"nav"` in that case — see `MviEntryCodegen`'s classification).
 */
internal data class VmCtorParam(
  val name: String,
  /**
   * Best-effort flattened FQ of the param type; may be unresolved (an as-yet-ungenerated
   * navigator).
   */
  val typeFq: String,
  /**
   * `true` if annotated `@dagger.assisted.Assisted` (Hilt) or
   * `@org.koin.core.annotation.InjectedParam` (Koin).
   */
 ...
 * AND as a KotlinPoet [TypeName] (for codegen — `StateFlow<S>`, VM references, etc.). The FQ string
 * alone flattens generics (`List<String>` → `kotlin.collections.List`); the [TypeName] preserves
 * the full parameterization. Capturing both avoids re-introducing the "generic-flattening" bug the
 * Sample Showcase stage fixed for ctor params (mirrors [dev.gezgin.processor.model.ParamModel]).
 */
