 ...
 * - `FeedScreen(route: Feed, nav: FeedNavigator)` — full core-mode shape: explicit
 *   `@Screen(Feed::class)` route + nav; the `route:` param carries route data and its type matches
 ...
 * - `AboutScreen(route: About)` — no `nav:` param — `About` has NO navigator ([SHOP_SOURCE]'s bare
 *   `@NoBack` route); pins that a `route`-only composable never needs one.
