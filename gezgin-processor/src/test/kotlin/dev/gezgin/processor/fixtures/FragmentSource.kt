...
/**
 * Task 6.1 fixtures for [dev.gezgin.processor.FragmentModelReaderTest] — brownfield Fragment
 * interop (§11/§11.1/§11.2).
 *
 * **Fragment stub strategy (documented decision, mirrors [HILT_STUBS]/[KOIN_STUBS]):**
 * `gezgin-processor` has NO compile dependency on `androidx.fragment` (its symbols are read as
 * string FQNs, and per §11.2 never will). So [FRAGMENT_STUB] declares a MINIMAL local `abstract
 * class Fragment` under the EXACT real FQN package `androidx.fragment.app` — the same
 * string-FQN-reading contract the processor uses in production, exercised against a
 * compile-time-present stub. A `@FragmentScreen`-annotated `class X : Fragment()` then type-checks
 * with a no-arg ctor exactly as a real Fragment subclass would.
 */
...
/**
 * Leaf routes for the Fragment fixtures, in a DIFFERENT package than the Fragments themselves — so
 * the dump proves `routePackageName` is read off the route DECLARATION (cross-module-safe), not the
 * Fragment's own package. `ArchivedRoute` is `@NoBack` (pins `noBack=true` read off the route
 * decl).
 */
val FRAGMENT_ROUTES =
  """
...
/**
 * Two well-formed `@FragmentScreen` leaves (§11.1 example shape) — no-arg ctors, mandatory route
 * arg. `OrderChainFragment` → route `OrderChainRoute` (x=`OrderChain`, noBack=false);
 * `ArchivedFragment` → `@NoBack ArchivedRoute` (x=`Archived`, noBack=true). Both routes live in
 * `dev.gezgin.fragroutes`.
 */
val FRAGMENT_SOURCE =
  """
...
/**
 * Stand-ins for the `XNavigator` classes [dev.gezgin.processor.codegen.NavigatorCodegen] would
 * generate in the ROUTES' OWN module. In a real multi-module build that module compiles first, so
 * its navigators are already-compiled classes on the Fragment module's classpath; the cross-module
 * nav-wiring PROBE ([dev.gezgin.processor.codegen.NavigatorProbe]) then resolves them BY NAME and
 ...
 * - `FeedFragment` → `HomeGraph.Feed`, which earns a `FeedNavigator` (has `@GoTo`/`@GoForResult`
 *   edges) → nav wiring MUST be emitted exactly as before (regression pin for the WITH-navigator
 *   case).
 * - `LockedFragment` → a deliberately-bare `@NoBack` route (no edges/back-edges/result-contract) →
 *   earns NO navigator → nav wiring MUST be SUPPRESSED (no `val nav`, no `lockedNavigator` import,
 *   2-arg `bindGezgin(fragment, route)`), the edge-less-leaf display-only case.
 */
