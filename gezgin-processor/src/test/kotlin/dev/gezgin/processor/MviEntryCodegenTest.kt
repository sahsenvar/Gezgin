...
/**
 * Faz 5.2 gate for [dev.gezgin.processor.codegen.MviEntryCodegen] — MVI-mode `provideXEntry`
 * codegen (spec §10.1): DI-detected default resolver, conditional nav wiring, route-explicit effect
 * wiring, and Problem-2 resolver params.
 *
 * **kctfork caveat (same as [EntryCodegenTest]):** the emitted entry bodies call compose-runtime
 * inline accessors (`collectAsStateWithLifecycle`, `CompositionLocal.current`) that the plugin-less
 * kctfork BACKEND can't inline (ICE); the Koin/Hilt fixtures additionally reference symbols absent
 * from the classpath. So these tests do NOT assert `exitCode == OK` — they assert no
 * `[SC*]`/`[MV*]` KSP error (codegen ran clean) plus the generated `GezginMviEntries.kt` golden
 * TEXT, which the successful KSP round populates regardless of the later backend/frontend failure.
 */
...
class MviEntryCodegenTest {
  ...
  fun `codegen-enabled overloaded Screen declarations fail with routes and function name`() {
    ...
    // content by simple name, which makes these two otherwise-valid overloads ambiguous in
    // generated code.
    val source =
  ...
  fun `route-local entries wrap shared ColumnScope content with only their own chrome`() {
    ...
    val routeBBody =
      text
        .substringAfter("fun GezginEntryScope.provideBEntry")
        .substringBefore("fun GezginEntryScope.")
  ...
  fun `bottom-only generated entry observes IME and preserves content-before-bottom order`() {
    ...
    val body =
      text
        .substringAfter("fun GezginEntryScope.provideAEntry")
        .substringBefore("fun GezginEntryScope.")
  ...
  fun `androidx-fallback default resolver — route-only ctor, effect wired, no nav`() {
    ...
    assertContains(
      text,
      "register<CounterRoute>(kind = EntryKind.SCREEN, noBack = false) { route ->",
    )
  ...
  fun `route-explicit EffectHandler may dispatch owner intents`() {
    ...
    assertContains(routeABody, "EffectsA(effects = vm.effects, nav = nav, onIntent = vm::onIntent)")
  }
  ...
  fun `Problem 2 — bottom sheet role extra (sheetState) and resolver extra (imageLoader)`() {
    ...
    assertContains(
      text,
      "register<SheetRoute>(kind = EntryKind.BOTTOM_SHEET, noBack = false) { route ->",
    )
    // M3 — the role is now Gezgin's own GezginSheetController (not the experimental material3
    // SheetState),
  ...
  fun `MJ1 — nav-named non-navigator @InjectedParam is OTHER not NAV (no spurious MV7, no default)`() {
    ...
    assertFalse(
      text.contains("koinViewModel"),
      "no default should be emitted (nav is OTHER): $text",
    )
  ...
  fun `MN4a — a defaulted OTHER ctor param is honored (default resolver still emitted, param omitted)`() {
    // `retries: Int = 3` is an OTHER param but HAS a default → it must NOT force the `viewModel`
    // param
    // to become required. A default resolver is still emitted; the ctor call omits `retries` (named
    // args).
    val text =
      generateMvi(
        SourceFile.kotlin(
    ...
    assertFalse(
      ...
      "defaulted OTHER param must be omitted from the ctor call: $text",
    )
  }
  ...
  fun `MN4b — a defaulted content extra needs no resolver param (uses the composable's own default)`() {
    ...
    assertFalse(
      text.contains("showTitle"),
      "defaulted content extra must not force a resolver: $text",
    )
