 ...
 * supertype relationship, not lexical nesting. Also pins the new parent-structure validations (N11
 * belirsiz ebeveyn, N12 öksüz `@FlowGraph`) and confirms the existing N10 simple-name collision
...
class FlatFileGraphTest {
  ...
  fun `membership derives from supertypes across files`() {
    ...
    // AppGraph's members are discovered cross-file: the separate-file route AND the separate-file
    // flow.
    assertContains(
      dump,
      ...
      message =
        "AppGraph must own its cross-file route + flow via `: AppGraph`:\n${dump.joinToString("\n")}",
    )
  ...
  fun `flat-file graph generates topology, serializers and navigators end-to-end`() {
    ...
    val topology =
      loader
        .loadClass("dev.gezgin.flat.GezginGeneratedKt")
    ...
    // M2 — compare the flow chain by public (id, isResultFlow) fields (FlowType is no longer a data
    // class).
    assertEquals(
    ...
    // M2 — `edges` is now internal; the cross-file HomeRoute→SignUpFlow result edge + its
    // serializer are
    // proven by the generated CredentialsNavigator `quitWith(result: SignUpResult)` assertion
    // below.

    // Serializers: the polymorphic subclass set spans both files (top-level route + nested flow
    // route).
  ...
  fun `cross-file sealed or abstract intermediate types under a graph are not routes`() {
    ...
    val routeLines =
      findGeneratedResource("GezginModelDump.txt")!!.readText().lines().filter {
        it.startsWith("route ")
      }
    assertFalse(
