...
/**
 * Task 6.1 gate for [dev.gezgin.processor.fragment.FragmentModelReader] (spec §11/§11.1/§11.2
 * brownfield Fragment interop). Positive cases assert the exact `GezginFragmentDump.txt` produced
 * under `gezgin.dumpFragment=true`; guardrail cases (`FS1`-`FS3`) assert a `[<code>]`-prefixed KSP
 * error.
 *
 * Fragment interop is screen-only (§11.2 — no dialog/bottom-sheet Fragment variant).
 * `FragmentModelReader` produces NO codegen in 6.1 (that's 6.2's `AndroidFragment` wiring), so the
 * model dump is the golden surface here. The `androidx.fragment.app.Fragment` the fixtures extend
 * is a local stub (see [dev.gezgin.processor.fixtures.FRAGMENT_STUB]) — `gezgin-processor` reads
 * fragment symbols as string FQNs with no real compile dependency.
 */
...
class FragmentModelReaderTest {
  ...
  fun `FS4 — a @FragmentScreen and a core-mode @Screen resolving to the same X in one package (cross-kind)`() {
    // The load-bearing cross-kind case: a core @Screen (provideDetailEntry via GezginEntries.kt)
    // and a
    // @FragmentScreen whose route also derives X="Detail" (provideDetailEntry via
