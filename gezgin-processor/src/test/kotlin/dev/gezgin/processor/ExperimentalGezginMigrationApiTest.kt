...
class ExperimentalGezginMigrationApiTest {
  ...
  fun `temporary migration APIs require explicit opt-in`() {
    migrationReferences().forEach { (name, declaration) ->
      val result =
        compileGezgin(SourceFile.kotlin("MigrationApi${name}.kt", migrationSource(declaration)))
  ...
  private fun migrationReferences(): List<Pair<String, String>> =
    listOf(
      ...
      "DragHandleContract" to
        "fun dragHandle(contract: BottomSheetContract) = contract.dragHandleMode",
    )
