...
/**
 * Task 3.4 gate for
 * [dev.gezgin.processor.codegen.EntryCodegen]/[dev.gezgin.processor.entry.EntryModelReader] —
 * core-mode `provideXEntry` codegen (spec §10.1/§12/§14).
 *
 * **Compose-in-kctfork finding (Task 3.4 brief's open question):** kctfork's test compilation has
 * NO compose-compiler plugin registered (`:gezgin-processor` is a plain `kotlin.jvm` module) — but
 * `@Composable` is JUST an annotation to the FRONTEND; the "only call composables from a composable
 * context" restriction is enforced by the compose-compiler plugin's FIR checker, which isn't
 * present here. So composable stubs (and [dev.gezgin.processor.codegen.EntryCodegen]'s generated
 * `provideXEntry` bodies, which call composables from inside a plain lambda) type-check as
 * perfectly ordinary Kotlin — no `gezgin.emitEntries=false` escape hatch needed for the FRONTEND.
 * The BACKEND, however, independently ICEs ("Couldn't inline method call: ...
 * `CompositionLocal.current` [inline] ... Method: null") trying to inline
 * `androidx.compose.runtime.CompositionLocal.current` from the compose-runtime dependency jar — a
 * kotlin-compile-testing sandbox limitation (real Gradle/AGP builds compile this exact
 * `CompositionLocal.current` read pattern everywhere, with no such issue), not a defect in the
 * generated code. So this test does NOT assert `exitCode == OK`; it only asserts no `[SC*]`/KSP
 * error surfaced (i.e. the codegen itself ran clean) plus the generated source text (golden) —
 * `sourcesGeneratedBySymbolProcessor` is populated by the earlier, successful KSP round regardless
 * of the later backend crash.
 */
...
class EntryCodegenTest {
  ...
  fun `provideXEntry golden text — route+nav, route-only, nav-only, dialog kind`() {
    ...
    assertTrue(
      text.contains(
        "val nav = LocalGezginRawNavigator.current.feedNavigator(LocalGezginEntryId.current)"
      ),
      text,
    )
    ...
    assertTrue(
      text.contains(
        "register<HomeGraph.Promo>(kind = EntryKind.DIALOG, noBack = false) { route ->"
      ),
      text,
    )
    ...
    assertTrue(
      text.contains(
        "register<HomeGraph.Product>(kind = EntryKind.SCREEN, noBack = true) { route ->"
      ),
      text,
    )
  ...
  fun `gezgin_emitEntries=false suppresses GezginEntries generation`() {
    ...
    assertTrue(
      ...
      "GezginEntries.kt emitted despite opt-out",
    )
  }
  ...
  fun `cross-module — feature with NO graphs still emits entries and factory import resolves to the route's package`() {
    ...
    assertTrue(
      feature.generatedSourceFor("FeedNavigator.kt") == null,
      "feature re-emitted a navigator",
    )
  }
  ...
  fun `SC9 — explicit Route class sentinel is rejected (route must name a concrete destination)`() {
    ...
    // concrete destination and must be rejected with an actionable "write the target route"
    // message.
    val source =
      """
  ...
  // class
  // before KSP runs. The check survives in EntryModelReader as a defensive guard against error
  // types
  ...
  fun `SC7 — @NoBack + @BottomSheet without BottomSheetContract is rejected at KSP`() {
    // No contract means both runtime values use their unsafe true defaults, which is statically
    // known.
    val msg =
      messagesOf(
        """
  ...
  fun `SC7 allow — @NoBack + @BottomSheet WITH BottomSheetContract reaches runtime value guard`() {
    // Getter results are route-instance runtime values. KSP may validate only the structural
    // presence
  ...
  fun `SC8 — @FullscreenModal route implementing DialogContract is rejected (mismatch)`() {
    ...
    // `route as? FullscreenModalContract` = null → type defaults. Reject the kind↔contract
    // mismatch.
    val msg =
      messagesOf(
        """
  ...
  fun `SC2 — nav param typed as a resolvable NON-navigator (RawNavigator) is rejected (Integ m4)`() {
    // `Feed` DOES earn a FeedNavigator (the hasNavigator check passes), but the `nav:` param is
    // typed
