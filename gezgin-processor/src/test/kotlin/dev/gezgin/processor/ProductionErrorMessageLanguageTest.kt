...
class ProductionErrorMessageLanguageTest {
  ...
  fun `production string literals are English`() {
    ...
    val offenders =
      sourceRoots.filter(Files::exists).flatMap(::kotlinFiles).flatMap { file ->
        extractStringLiterals(Files.readString(file))
          .filter { it.text.hasTurkishLetter() }
          .map { literal ->
  ...
  private fun repoRoot(): Path {
    ...
    while (!Files.exists(dir.resolve("settings.gradle.kts"))) {
      dir =
        requireNotNull(dir.parent) {
          "Could not find repository root from ${Paths.get("").toAbsolutePath()}"
        }
    }
    return dir
  }
