...
/**
 * Task 6.2 gate for [dev.gezgin.processor.codegen.FragmentEntryCodegen] — the
 * `AndroidFragment`-hosting `provideXEntry` codegen for `@FragmentScreen` (spec §11.1, screen-only
 * §11.2).
 *
 * **kctfork caveat (same as [EntryCodegenTest]/[MviEntryCodegenTest]):** the emitted body calls
 * `androidx.fragment.compose.AndroidFragment` (an inline `@Composable`, not on the classpath here)
 * and the gezgin-core `androidMain` `toBundle`/`bindGezgin` glue (androidMain, also not on the jvm
 * test classpath), plus the compose-runtime inline accessors the plugin-less kctfork BACKEND can't
 * inline — so this test does NOT assert `exitCode == OK`. It asserts no `[FS*]`/`[SC*]` KSP error
 * (codegen ran clean) plus the generated `GezginFragmentEntries.kt` golden TEXT, which the
 * successful KSP round populates regardless of the later compile failure
 * (`sourcesGeneratedBySymbolProcessor` reads the KSP output dir). Reuses the Task-6.1 fixtures —
 * routes in a DIFFERENT package than the Fragments, so cross-module factory qualification shows.
 */
...
class FragmentEntryCodegenTest {
  ...
  private fun generateFragmentEntries(): String {
    val result =
      compileGezgin(
        ...
        // Stand-in for the routes' cross-module (already-compiled) navigators, so the nav-wiring
        // probe
  ...
  fun `provideXEntry golden text — AndroidFragment host, toBundle args, onUpdate bind, cross-module probe nav`() {
    ...
    // K4 — the fragment entry file opts in to the generated-code gate (route.toBundle/bindGezgin
    // are gated).
    ...
    // OrderChain: these routes are NOT in this module's model (no graphs here) → the cross-module
    // branch
    ...
    // Archived: @NoBack route → register carries noBack = true (read off the route decl,
    // cross-module).
    assertContains(text, "public fun GezginEntryScope.provideArchivedEntry()")
    assertContains(
      text,
      "register<ArchivedRoute>(kind = EntryKind.SCREEN, noBack = true) { route ->",
    )
  ...
  fun `nav wiring is conditional — edge-less route emits no nav, route with navigator keeps it`() {
    ...
    // (b) WITH navigator — Feed earns a FeedNavigator (has @GoTo/@GoForResult edges): nav wiring
    // emitted
  ...
  fun `cross-module display-only Fragment — no compiled navigator emits no nav wiring (probe fix)`() {
    ...
    assertContains(
      text,
      "register<SettingsRoute>(kind = EntryKind.SCREEN, noBack = false) { route ->",
    )
    assertContains(text, "AndroidFragment<SettingsFragment>(")
    // NO navigator — the cross-module probe found no compiled SettingsNavigator → 2-arg no-nav
    // bindGezgin.
    assertContains(text, "onUpdate = { fragment -> bindGezgin(fragment, route) },")
    assertFalse(
