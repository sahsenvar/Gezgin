 ...
 * discovered subtype + its containing file via `logger.warn`; the tests assert on that log. A
 * second assertion validates the FALLBACK enumeration (scan a declaration's direct supertypes) so
 * the report can weigh both mechanisms.
 */
...
class Faz8SpikeTest {
  ...
  fun `getSealedSubclasses finds a flow sub-interface declared in another file`() {
    ...
    assertContains(
      ...
      message =
        "getSealedSubclasses(RootGraph) must find the @FlowGraph sub-interface in SignUpFlow.kt",
    )
  }
  ...
  fun `getSealedSubclasses recurses into a cross-file flow to find its member route`() {
    ...
    assertContains(
      ...
      message =
        "getSealedSubclasses(SignUpFlow) must find its @StartDestination member (same file as flow)",
    )
  }
  ...
  private class SealedProbeProcessor(private val environment: SymbolProcessorEnvironment) :
    SymbolProcessor {
