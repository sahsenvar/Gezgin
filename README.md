...
Apply the KSP + serialization plugins and add the artifacts (`group = dev.gezgin`, `version = 0.1.0-alpha05`):
...
dependencies {
    ...
    // implementation("dev.gezgin:gezgin-mvi:0.1.0-alpha05")        // optional MVI add-on
    // testImplementation("dev.gezgin:gezgin-test:0.1.0-alpha05")   // UI-less testing: GezginTestNavigator + typed fromX()
}
```
