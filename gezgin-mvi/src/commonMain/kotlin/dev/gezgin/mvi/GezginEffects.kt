 ...
 * subscriber** nowhere. In Nav3 a covered entry leaves the composition ENTIRELY → no collector; or
 ...
 * overflow — back-to-back effects in one frame) `tryEmit` returns `false` and the effect is dropped
 * anyway — and common fixtures ignore the return value. Result: when the user comes back the
 ...
 * without a subscriber, [send]-ed effects wait in the queue (`UNLIMITED` → [send] never
 ...
 * delivery; that is why [flow] is for a **single** collector (`receiveAsFlow` does not fan out — a
 ...
 * therefore WRONG — the key would change on every recomposition.)
 *
 * Usage:
 * ```
...
public class GezginEffects<E> {
  ...
  /**
   * The lossless effect stream — assigned directly to [GezginMvi.effects]. A stable (`val`)
   * instance; for a **single** collector (an effect = exactly-once delivery, no fan-out).
   */
  public val flow: Flow<E> = channel.receiveAsFlow()

  /**
   * Send an effect. The `UNLIMITED` buffer → never suspends, never drops: even without an observer
