...
public interface GezginMvi<out S, in I, out E> {
   ...
   *
   * **Backing contract (losslessness, Faz 5 recheck / MJ2):** a buffered source is REQUIRED so that
   * an effect emitted while there is no observer is NOT LOST. RECOMMENDED: [GezginEffects]
   * (`Channel(UNLIMITED) .receiveAsFlow()`). The `MutableSharedFlow(replay = 0) + tryEmit` pattern
   * SILENTLY drops an effect while there is no observer (see the rationale in the [GezginEffects]
   * KDoc). The returned `Flow` must also be a **stable instance** (a single `val` — NOT a `get()`
   * that calls `receiveAsFlow()` on every access), otherwise [ObserveEffects]'s `LaunchedEffect`
   * key changes on every recomposition and restarts the collector.
   */
