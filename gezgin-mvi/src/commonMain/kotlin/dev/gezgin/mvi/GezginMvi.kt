 ...
 *
 * `@MviViewModel(Route::class)` işaretli VM bunu implement ETMELİDİR (İKİSİ DE ZORUNLU — guardrail, §10.1):
 * codegen VM'in somut tipini `@MviViewModel`'den, **S/I/E'yi bu supertype'ın tip arg'larından** okur
 * (content'ten türetme yok → spec'in E-kaynağı problemi çözülür). `@MviViewModel` var ama `GezginMvi`
...
public interface GezginMvi<out S, in I, out E> {
     ...
     * `emptyFlow()`. `@ScreenEffect` bunu [ObserveEffects] ile **yalnız STARTED iken** tüketir → örtülen
     ...
     * [ObserveEffects]'in `LaunchedEffect` key'i her recomposition'da değişip collector'ı restart eder.
     */
