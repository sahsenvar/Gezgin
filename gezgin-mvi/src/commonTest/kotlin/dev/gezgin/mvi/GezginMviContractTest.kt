...
/**
 * Task-5.0: `@MviViewModel` + `GezginMvi<S,I,E>` sözleşmesinin birlikte derlendiğini VE S/I/E
 * generic şeklinin çalışır olduğunu kanıtlayan minimal runnable test (compose'suz, saf Kotlin — §13
 * ruhu). Codegen okuması (S/I/E extraction, guardrail) 5.1'de; bu yalnız runtime sözleşmesini
 * doğrular.
 */
class GezginMviContractTest {
  ...
  fun effects_flow_emits_on_intent() = runTest {
    ...
    // Kayıpsız GezginEffects (Channel) backing'iyle collector emit'ten önce de sonra da
    // bağlanabilir;
    // burada önce bağlanıp gerçek-zamanlı teslim doğrulanır (UnconfinedTestDispatcher eager
    // collect).
    val job =
  ...
  fun effects_emitted_while_unobserved_are_delivered_on_reobservation() = runTest {
    ...
    assertEquals(listOf("count=1", "count=2"), received.map { (it as CounterEffect.Toast).text })
    job.cancel()
  }
}
