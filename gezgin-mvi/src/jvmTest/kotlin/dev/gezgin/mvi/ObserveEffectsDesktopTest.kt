 ...
 * `LocalLifecycleOwner`'ı `runComposeUiTest` üzerinden sağlar (JVM/desktop, cihazsız). Owner STARTED'a
 * çıkmasaydı `repeatOnLifecycle(STARTED)` HİÇ açılmaz, state başlangıç değerinde donar ve efekt HİÇ teslim
 * edilmezdi — bu testler o boşluğu (audit MN-A) doğrudan koşarak kapatır.
 */
...
class ObserveEffectsDesktopTest {
    ...
    // TestMainDispatcher ile değiştirir; `setMain` çağrılmadan `Main.immediate`'a erişmek (repeatOnLifecycle
    // + collectAsStateWithLifecycle içinde) fırlatır. runComposeUiTest bir Main KURMADIĞINDAN burada elle
    // kuruyoruz — böylece gerçek host-sürülen lifecycle akışını (STARTED-gate) cihazsız koşabiliyoruz.
    @BeforeTest
    fun installMain() = Dispatchers.setMain(Dispatchers.Unconfined)

    @AfterTest
    fun clearMain() = Dispatchers.resetMain()

    /** MN-A: state gerçekten render olur (collectAsStateWithLifecycle) VE efekt ObserveEffects'le gözlenir. */
    @Test
    ...
    /**
     * MJ-A (RED→GREEN): recomposition'da `onEffect` yeni bir lambda ile değişince (effects + lifecycleOwner
     * stabil olduğundan LaunchedEffect RESTART ETMEZ), SONRADAN yayılan efekt YENİ lambda'ya gitmeli.
     * Fix'siz (`collect(onEffect)`) collector İLK composition'ın lambda'sını sonsuza dek bağlar →
     * `handler=0` yakalayan bayat lambda çağrılır ("h0:count=1") ve bu assert KIRMIZI olurdu.
     */
    @Test
    fun `onEffect swap sonrasi yayilan efekt YENI lambda'ya gider (stale-lambda fix)`() = runComposeUiTest {
        val vm = CounterViewModel(CounterRoute(start = 0))
