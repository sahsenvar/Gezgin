...
class GezginTestNavigatorTest {
  ...
  @Test
  fun replaceToClearsPaymentFlow() {
  ...
  // §13 by-example — brief verbatim.
  @Test
  fun checkoutReturnsSelectedResult() = runTest {
  ...
  @Test
  fun backToDelegatePopsToTarget() {
    val nav = GezginTestNavigator(start = Catalog, topology = testTopology)
  ...
  @Test
  fun quitWithDelegateDeliversValue() = runTest {
    val nav = GezginTestNavigator(start = Catalog, topology = testTopology)
    val r = async { nav.raw.navigateForResult<OrderId>("Catalog→CheckoutFlow", Cart) }
    runCurrent()
  ...
  @Test
  fun backAtRootInvokesOnRootBack() {
    var rootBackCalled = false
    val nav =
      GezginTestNavigator(
        start = Catalog,
        topology = testTopology,
        onRootBack = { rootBackCalled = true },
      )
    nav.back()
  ...
  @Test
  fun navigateGrowsBackStackAndUpdatesCurrent() {
    val nav = GezginTestNavigator(start = Catalog, topology = testTopology)
    assertEquals(1, nav.backStack.size)
    nav.navigate(Product("p1"))
  ...
  @Test
  fun entryIdOfReturnsNearestEntryId() {
    val nav = GezginTestNavigator(start = Catalog, topology = testTopology)
    nav.navigate(Product("p1"))
    nav.navigate(Product("p2"))
  ...
  @Test
  fun entryIdOfThrowsExplanatoryErrorWhenNoMatch() {
    val nav = GezginTestNavigator(start = Catalog, topology = testTopology)
    val ex = assertFailsWith<IllegalStateException> { nav.entryIdOf(Payment::class) }
    assertTrue(ex.message.orEmpty().contains("Payment"))
  }

  // Faz 8.1 — the fixture's `CheckoutFlow` is FLAT-FILE-shaped: a top-level flow FQ
  // ("CheckoutFlow")
  ...
  @Test
  fun flatFileFlowQuitUnwindsWholeFlow() {
    val nav = GezginTestNavigator(start = Catalog, topology = testTopology)
  ...
  @Test
  fun flatFileResultFlowQuitWithDeliversValueAndUnwinds() = runTest {
    val nav = GezginTestNavigator(start = Catalog, topology = testTopology)
    val r = async { nav.raw.navigateForResult<OrderId>("Catalog→CheckoutFlow", Cart) }
    runCurrent()
