#!/usr/bin/env bash
# Madde 17b — nested ResultFlow'un ORTASINDA (Zoom, en derin) gerçek process-death, sonra akışı tamamla.
# Madde 4/15'in İKİ reçetesini nested-flow yaprağı için tekrarlar (her biri stack'i baştan tohumlar):
...
# İSPAT: nested AvatarFlow + pending result slot (pickAvatarResults) GERÇEK process ölümünden sağ çıkar —
#   restore sonrası hâlâ Zoom'dayız (flowChain+flowPath) ve quitWith sonucu Profile'a teslim edilir (pendingSlots).
...
echo "== [CASE A / DKA] nested akışın derinine (Zoom) yürü =="
mt "$DIR/app-17b-avatar-to-zoom.yaml" || fail=1
...
if [ "$(adb shell settings get global always_finish_activities 2>/dev/null | tr -d '\r')" = "1" ]; then
  ...
  sleep 2
  relaunch
  echo "== [A/restore] hâlâ Zoom mu + quitWith -> Profile teslim doğrula =="
  mt "$DIR/app-17b-avatar-after-pd.yaml" || fail=1
else
...
echo "== [CASE B / am kill] nested akışın derinine (Zoom) yeniden yürü =="
mt "$DIR/app-17b-avatar-to-zoom.yaml" || fail=1
...
echo "== [B/restore] fresh-process: hâlâ Zoom mu + quitWith -> Profile teslim doğrula =="
mt "$DIR/app-17b-avatar-after-pd.yaml" || fail=1

if [ "$fail" = "0" ]; then echo "MADDE 17b: PASS"; else echo "MADDE 17b: FAIL"; fi
exit $fail
