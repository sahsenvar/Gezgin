...
- **Host = `GezginDisplay(...)`** — ham `NavDisplay`'i kullanıcı yazmaz. GİZLİ iç plumbing: decorators (scoping/saved-state), deeplink resolver, entryProvider assembly, predictive-back, middleware pipeline. AÇIK parametreler: `navigator`, `entries`, app-geneli `transitions`, `middleware` (OkHttp interceptor gibi). Aynı zamanda **Nav3 adapter sınırı** (churn'ü gizler).
...
- **MVI'ı core'dan ayır:** `@ViewModel`/`GezginMvi`/`@ScreenEffect`/binder-codegen/DI-detection → **`gezgin-mvi`** (opt-in). `gezgin-core` DI/VM/MVI bilmez (`@Screen(route,nav)` + navigator'lar + routes/deeplink/topology + `GezginDisplay` + `EntryProviderScope`). Bağlantı seam'i: core'un `EntryProviderScope` + navigator'ları; mvi = entry-producer add-on. `gezgin-mvi → gezgin-core` bağımlı.
- **`GezginEntry` kaldırıldı** (over-engineering'di): `provideXEntry` = `EntryProviderScope` extension, Nav3'ün `entry<Route>{}`'sini **doğrudan** register eder (Zad `registerAll(scope)` deseni). Bundle (`xFeatureEntries()`) = **kullanıcı-yazımı** (codegen üretirse resolver override noktası kapanır); codegen yalnız bireysel `provideXEntry`'leri üretir. `GezginDisplay { … }` trailing lambda = `EntryProviderScope<NavKey>`.
