...
**Not (checklist vs implementasyon):** EditName step-2'nin "ismi SİL → dışarı tık kapanmaz" beklentisi sample'da GEÇERSİZ — `dismissOnClickOutside = route.current.isNotBlank()` route ARGÜMANINDAN (açılışta sabit) okunur, canlı text-field'dan değil. Profil adı boş olmadığı için dış-tık HER ZAMAN kapatır. (Kütüphane doğru; checklist prose'u idealize bir senaryo tarif ediyor.)
...
**Durum:** [x] Result yarısı Maestro ile doğrulandı / [ ] animasyon akıcılığı görsel — insan gözü gerekir. `maestro/app-10c-filter-sheet.yaml`: seçenek seçildiğinde **hide-then-result** sırası doğrulandı (sheet önce kapanır, order SONRA güncellenir — programatik animasyonsuz kapanış GÖRÜLMEZ). Slide-down animasyonunun akıcılığı/aniden-kesilmemesi assertion'la ölçülemez → görsel.
