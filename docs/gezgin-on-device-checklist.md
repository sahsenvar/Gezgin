...
**Durum:** [x] Maestro ile doğrulandı (emülatör) — `maestro/run-15-fragment-pd.sh` ("Etkinlikleri saklama" round-trip): PD sonrası (a) crash/ANR YOK, HelpFragment **aynı derinlikte** restore (Login/Dashboard'a sıfırlanma yok); (b) `gezginArgs<HelpScreenRoute>()` route'u restore sonrası doğru decode etti → "Konu: navigasyon" (`route.toBundle()` PD-safe CANLI kanıt); (c) `gezginNav<HelpNavigator>()` re-bind çalıştı → "Panoya dön" (`backToDashboard()`) Dashboard'a döndü ("bind yok" hatası YOK — `AndroidFragment.onUpdate→bindGezgin` re-bind kanıtlandı).
...
| 15 | PD "Etkinlikleri saklama" — `@FragmentScreen` (`HelpFragment`; args decode + `onUpdate` re-bind) | Evet | [x] Maestro+adb (args decode + nav re-bind) |
