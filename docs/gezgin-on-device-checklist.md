...
**Durum:** [x] Maestro ile doğrulandı (emülatör, API 34+) — `maestro/shopr-01-noback-backto.yaml`: OrderPlaced (@NoBack terminal) ekranında sistem geri tuşu **2× no-op** (ekrandan çıkılmaz, önceki ekranlara dönülmez); ardından "Feed'e dön" (@BackTo) Feed'e döner. Predictive-back preview'ın hiç **başlamaması** kalemi görsel — insan gözü gerekir.
...
`WelcomeScreenRoute`'un bir forward-edge'i VAR (`@ReplaceTo(DashboardScreenRoute, name = "continueToDashboard")`,
`sample/navigation/.../HomeGraph.kt:50`) — ama bu edge bir MODAL açmaz, yalnızca Dashboard'a replace eder;
Welcome'ın üstünde açılan declared bir modal edge YOKTUR, dolayısıyla "modal `@NoBack` entry'nin üstünde"
senaryosu bu topolojide DOĞRUDAN kurulamıyor (kapsam-dışılığın gerçek sebebi budur — "forward-edge yok"
DEĞİL). Genel modal-dismiss/geri-tuşu davranışları (tap-outside/swipe/back →
...
**Durum:** [x] Maestro ile doğrulandı (genel dismiss davranışı, emülatör) — `maestro/app-02-modal-back-dismiss.yaml`: ForgotPasswordDialog sistem-back ile kapanır, LoginScreen geri gelir ve **etkileşilebilir** (dialog tekrar açılabiliyor). Spesifik `@NoBack`-altında-modal senaryosu hâlâ kapsam dışı (sample topolojisinde yok).
...
**Durum:** [x] Maestro ile doğrulandı (emülatör, KOD DEĞİŞİKLİĞİ GEREKMEDEN) — `maestro/app-03-r2-vmstore.yaml`: ItemDetail'in `@MviViewModel`'i + "İlgili ürün (aynı id, yeni entry)" (`@GoTo(singleTop=false)`) ile aynı route DEĞERİ (id=item-1) iki kez açıldı; `visits` sayacı dizisi **1 → 1 → 2** (entry #2 kendi taze VM'ini alır = paylaşılmıyor; geri dönünce entry #1 kendi RETAINED store'uyla 2'ye çıkar). Paylaşılan VM olsaydı 1→2→3 olurdu. Bu, R2'nin VM-store yarısının cihaz kanıtı (checklist'in "geçici viewModel + Log ekle" reçetesine gerek kalmadı).
...
**Durum:** [ ] görsel — insan gözü gerekir. Scrim opaklığı, tek-katman z-order, iki-modal arası flicker/kalıntı yalnız gerçek compositor'da göz ile değerlendirilebilir; assertion'la doğrulanamaz. (Modalların FONKSİYONEL açılış/dismiss'i madde 10/11/13'te Maestro ile doğrulandı.)
...
**Durum:** [x] Maestro ile doğrulandı (emülatör) — `maestro/app-10a-forgot-dialog-canceled.yaml`, `maestro/app-10b-editname-dialog.yaml`, `maestro/app-10c-filter-sheet.yaml`:
- ForgotDialog: dışarı/scrim tık `dismissOnClickOutside=false` → **no-op** (açık kalır); sistem-back → "Sıfırlama iptal edildi" (Canceled); "Gönder" → "Sıfırlama linki gönderildi" (Value=true).
- EditName: dışarı tık → dialog kapanır + ad DEĞİŞMEZ (Canceled); "Kaydet" (ismi düzenleyip) → "Profil: Yeni Ad" + "Ad güncellendi" (Value).
- FilterSheet: aşağı swipe → kapanır + order sabit (Canceled); scrim tık → aynı; seçenek seç → **hide-then-result** (sheet önce kapanır, sonra order=PRICE_ASC).

**Not (checklist vs implementasyon):** EditName step-2'nin "ismi SİL → dışarı tık kapanmaz" beklentisi sample'da GEÇERSİZ — `dismissOnClickOutside = route.current.isNotBlank()` route ARGÜMANINDAN (açılışta sabit) okunur, canlı text-field'dan değil. Profil adı boş olmadığı için dış-tık HER ZAMAN kapatır. **Yapısal sebep:** dialog dismiss/layout prop'ları (`dismissOnClickOutside`/`dismissOnBackPress`/`usePlatformDefaultWidth`) entry KURULURKEN BİR KEZ çözülür (open-time sabitleri) → canlı state okuyan bir getter (`current.isNotBlank()`) bile `dismissOnClickOutside`'ı canlı düzenlemelere TEPKİ VERECEK şekilde değiştiremez; bu, dokümante bir sözleşme özelliğidir, bir BUG DEĞİL (eşleşen tek-satır KDoc `gezgin-core` `Contracts.kt`'ye ekleniyor). (Kütüphane doğru; checklist prose'u idealize bir senaryo tarif ediyor.)
...
**Durum:** [x] Davranışsal yarı Maestro ile doğrulandı (emülatör) — `maestro/app-11-back-gesture-modal.yaml`: sol-kenar tam geri-JESTİ dialog'u kapatır (modal'ın `onDismissRequest`'ine gider, LoginScreen navige OLMAZ) ve sheet'i kapatır (Canceled, Dashboard order sabit). Predictive-back **preview-frame** görselleri (yarım-jest önizlemesi, alttaki ekranda preview başlamaması) görsel — insan gözü gerekir.
...
**Durum:** [x] Result yarısı Maestro ile doğrulandı / [ ] animasyon akıcılığı görsel — insan gözü gerekir. `maestro/app-10c-filter-sheet.yaml`: seçenek seçildiğinde **result-delivery + kapalı-durum** doğrulandı (final durum: sheet KAPALI VE order=PRICE_ASC güncellenmiş). **Sıra ölçülemedi:** flow yalnızca FINAL durumu assert eder → `hide()`→result SIRASINI (sheet önce mi kapandı, sonuç sonra mı geldi) gözleyemez; bu sıralama final-state assertion'larıyla ölçülemez (görsel gözlem ya da logcat-timestamp probe gerekir). Slide-down animasyonunun akıcılığı/aniden-kesilmemesi de assertion'la ölçülemez → görsel.
...
**Durum:** [x] Maestro ile doğrulandı (emülatör) — `maestro/app-13-fullscreen-modal.yaml`: ImageViewer açıkken alttaki ItemDetail'in kendine-özgü butonları ("Panoya dön", "Görseli tam ekran gör") **assertNotVisible** (tam-ekran occlusion; dialog/sheet'te bunlar erişilebilir kalırdı → occlusion'ın davranışsal proxy'si); sistem-back → kapanır, ItemDetail geri gelir; "Kapat" → `backToItemDetail()` ile ItemDetail'e döner. (Tam-ekran içeriğinin dışında "outside" bölge olmadığından dış-tık adımı doğal olarak N/A; kenar-boşluğu YOK olması occlusion assertion'ıyla kapsanıyor.)
...
- "Koyu tema" toggle → switch AÇIK + Toast; efekt logcat'te **tam 1 kez** (`SettingsMvi effect`) → tek-seferlik.
- Landscape'e döndürüldü → switch AÇIK **korundu** (VM config-change'i sağ kaldı, `remember` değil); efekt sayısı **hâlâ 1** (rotation replay YOK).
- "Çıkış yap" → Login; geri tuşu Dashboard'a/Settings'e **dönmez**.

**CİHAZ BULGUSU (davranış testi vs gerçek app):** logout SONRASI cihazda **TEK** Login entry var, "çift Login" DEĞİL — sistem-back Login'den kök-back → app KAPANIR. Sebep: `loginSuccess = @ReplaceTo(Dashboard)` seed Login'i tüketir (stack `[Dashboard]`), böylece `logout = @ReplaceTo(Login, clearUpTo=Dashboard, inclusive)` → tek `[Login]`. Birim testi `AppNavBehaviorTest.logout…stacksASecondLoginEntry` `[Login,Login]` bekler ama login'i düz `navigate(Dashboard)` PUSH'uyla kurar — gerçek app'in @ReplaceTo yolunu YANSITMAZ. Kütüphane doğru; test kurulumu temsili değil. Güvenlik invariant'ı (back Dashboard/Settings'e dönmez) korunur. Bir KÜTÜPHANE bug'ı DEĞİL.
...
**Durum:** [x] Maestro ile doğrulandı — DKA VE gerçek process-death (am kill) PASS (2026-07-12, emulator-5554; fix sonrası).
- **CASE A — DKA (`always_finish_activities`, process YAŞAR):** `maestro/run-15-fragment-pd.sh` CASE A PASS — (a) crash/ANR YOK, HelpFragment aynı derinlikte restore; (b) `gezginArgs<HelpScreenRoute>()` → "Konu: navigasyon"; (c) `gezginNav<HelpNavigator>()` re-bind → "Panoya dön" Dashboard'a döndü.
- **CASE B — GERÇEK process-death (`am kill`, fresh process):** PASS (fix sonrası; pid 14215→14338 = gerçek ölüm+yeniden doğuş). Restore SONRASI `gezginArgs` "Konu: navigasyon" doğru decode etti, `gezginNav` re-bind "Panoya dön" Dashboard'a döndü — crash YOK.
- **BULUNAN + DÜZELTİLEN V1 BUG (P0.2 am-kill sertleştirmesinin kanıtı):** Bu case ÖNCE crash ediyordu — `IllegalStateException: ... route.toBundle() ... before gezginArgs was read`. Kök neden: taze process'te FragmentManager, Fragment'ı `Activity.onCreate super`'inde — `setContent` kompozisyonundan (dolayısıyla `route.toBundle`'ın statik `gezginFragmentJson`'u doldurmasından) ÖNCE — `onViewCreated`'a kadar restore eder → statik `null` → decode edecek app-Json yok. DKA process'i yaşatınca statik dolu kalıyor, bu yüzden yalnız gerçek PD açığa çıkarır. **Fix:** yeni public `Gezgin.initFragmentInterop(gezginJson)` app-Json'u process açılışında (`Application.onCreate`, FM restore'dan ÖNCE) kaydeder; sample `ShowcaseApp` bunu çağırır. `FragmentBinding.android.kt` KDoc/error da düzeltildi (eski "onViewCreated her iki branch'te güvenli" iddiası yanlıştı).

**Kapsam notu (DKA ≠ gerçek process-death; (b) hangi branch koştu):** madde 4 ile aynı — bu round-trip `always_finish_activities` (DKA); **process YAŞADIĞI** için statik `gezginFragmentJson` (`FragmentRouteBundle.android.kt`) hayatta kalır, yani (b) restore'u statik-CANLI yoldan koştu. `FragmentBinding.android.kt:24-31` KDoc'u iki yolu ayırır: (i) `AndroidFragment`'ın Fragment'ı kendi örnekleyip `route.toBundle`'ın statiği ilk kompozisyondan önce doldurduğu **ilk-yaratım/re-encode** yolu (statik dolu), (ii) taze process'te statik `null`'ken FragmentManager'ın saved-Fragment'ı ilk kompozisyondan ÖNCE geri yüklediği **fresh-process FM-restore** yolu (statik `null` → yeniden doldurulur). KDoc'un asıl riskli saydığı (ii) fresh-process yolu yalnızca gerçek cold-process PD'de tetiklenir; DKA process'i öldürmediği için **koşulmadı** — `am kill` + pid-değişimi varyantı (gap raporu P0.2) gelene dek doğrulanmamış kalır.
...
| 13 | `@FullscreenModal` tam-ekran occlusion + dismiss (`ItemImageViewerRoute`) | Evet | [x] Maestro (occlusion proxy + dismiss) |
| 14 | MVI-mode: VM config-change ömrü + tek-seferlik efekt + MVI'dan logout | Evet | [x] Maestro+adb (BULGU: cihazda tek Login, çift değil) |
| 15 | PD "Etkinlikleri saklama" — `@FragmentScreen` (`HelpFragment`; args decode + `onUpdate` re-bind) | Evet | [x] Maestro — DKA + gerçek PD (am kill) PASS (V1 bug bulundu+düzeltildi: `Gezgin.initFragmentInterop`) |
| 16 | Legacy Fragment `OnBackPressedDispatcher` LIFO-bypass | Hayır (bilgilendirici) | [ ] Bilgilendirici |
| 17 | Migration-swap `@FragmentScreen` → `@Screen` (`HelpFragment`→`HelpScreen`) | Evet (kod dönüşümü) | [ ] Otomatikleştirilemez (kod dönüşümü+rebuild) |
