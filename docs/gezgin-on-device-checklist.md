...
**Durum:** [~] KISMİ — DKA (activity-recreation) PASS, gerçek process-death CRASH (2026-07-12, emulator-5554).
- **CASE A — DKA (`always_finish_activities`, process YAŞAR):** `maestro/run-15-fragment-pd.sh` CASE A PASS — (a) crash/ANR YOK, HelpFragment aynı derinlikte restore; (b) `gezginArgs<HelpScreenRoute>()` → "Konu: navigasyon"; (c) `gezginNav<HelpNavigator>()` re-bind → "Panoya dön" Dashboard'a döndü. (Statik `gezginFragmentJson` process-alive olduğundan dolu.)
- **CASE B — GERÇEK process-death (`am kill`, fresh process):** **CRASH** (deterministik 2/2). `IllegalStateException: No Gezgin route.toBundle() has been evaluated before gezginArgs was read` (`FragmentBinding.android.kt:180` ← `HelpFragment.onViewCreated:32`). **Kök neden:** taze process'te FragmentManager, HelpFragment'ı `Activity.onCreate`'in `super`'inde — `setContent` kompozisyonundan (dolayısıyla `route.toBundle`'ın statiği doldurmasından) ÖNCE — restore edip `onViewCreated`'a kadar dispatch eder → statik `gezginFragmentJson` HÂLÂ `null` → decode edecek app-Json yok → fırlatır. `FragmentBinding.android.kt` KDoc'unun "onViewCreated her iki branch'te güvenli" iddiası CİHAZDA YANLIŞLANDI (sample onViewCreated'da okuyor ve yine çöküyor). Bu, Fragment interop'un **gerçek process-death altında V1 bug'ı** — DKA'nın maskelediği, yalnız am-kill'in açtığı yol (P0.2 sertleştirmesinin bulduğu). **Fix kararı bekliyor** (bkz. gap raporu; seçenekler: startup-Initializer otomatik / açık app-init / V1-sınırı-dokümante).
...
| 15 | PD "Etkinlikleri saklama" — `@FragmentScreen` (`HelpFragment`; args decode + `onUpdate` re-bind) | Evet | [~] DKA PASS / gerçek PD (am kill) **CRASH** — V1 fragment-interop bug, fix bekliyor |
