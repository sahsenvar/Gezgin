...
data class PublicApiKDocFinding(
    val declaration: PublicApiDeclaration,
    val kind: KDocFindingKind,
)
...
data class KotlinPublicApiScanResult(
    ...
    val includedCount: Int get() = declarations.count { !it.excluded }
    val excludedCount: Int get() = declarations.count { it.excluded }
}

/**
 * Parses Kotlin into PSI and inventories effective-public declarations without relying on line layout or
 * modifier ordering. Only file declarations, public class members, and primary-constructor properties are
...
class KotlinPublicApiScanner : AutoCloseable {
    ...
    fun scan(input: KotlinSourceInput): KotlinPublicApiScanResult {
        ...
        val findings = declarations.flatMap { declaration ->
            if (declaration.excluded) {
                emptyList()
            } else {
                buildList {
                    if (!declaration.hasKDoc) add(PublicApiKDocFinding(declaration, KDocFindingKind.MISSING_KDOC))
    ...
    private fun KtDeclaration.effectiveKDoc(): KDoc? {
        ...
        return ownerDoc.takeIf { doc ->
            Regex("(?m)^\\s*\\*?\\s*@(property|param)\\s+${Regex.escape(propertyName)}\\b").containsMatchIn(doc.text)
        }
    }
