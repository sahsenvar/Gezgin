...
object ReleasePublicationVerifier {
  ...
  fun verify(repository: Path, requireSignatures: Boolean): VerificationSummary {
    ...
    check(Files.isDirectory(groupDirectory)) {
      "release group directory does not exist: $groupDirectory"
    }

    val actualCoordinates =
      Files.list(groupDirectory).use { paths ->
        paths
    ...
    if (requireSignatures) {
      ...
      val actualSignatures =
        Files.walk(groupDirectory).use { paths ->
          paths
      ...
      val relativeExpectedSignatures =
        expectedSignatures.mapTo(linkedSetOf()) { repository.relativize(it) }
      ...
      runCatching {
          JarFile(path.toFile()).use { archive ->
            archive.entries().asSequence().any { entry ->
              !entry.isDirectory && requiredEntry(entry.name)
            }
          }
        }
        .getOrElse { false }
  ...
  private fun verifyPom(path: Path, artifact: ExpectedArtifact) {
    ...
    val duplicateDependencies =
      actualDependencies.groupBy { "${it.group}:${it.artifact}" }.filterValues { it.size > 1 }.keys
    ...
    val expectedInternalDependencies =
      artifact.pomProjectDependency
  ...
  private fun verifyModuleMetadata(path: Path, artifact: ExpectedArtifact) {
    ...
    val expectedDependencies =
      artifact.moduleProjectDependency
  ...
  data class VerificationSummary(val coordinateCount: Int, val signatureCount: Int)
  ...
  private data class ExpectedArtifact(
    ...
    val expectedExternalDependencies: Set<PomDependency> =
  ...
  private fun descriptionFor(projectName: String): String =
    when (projectName) {
      "gezgin-core" ->
  ...
  private fun expectedExternalDependenciesFor(artifactId: String): Set<PomDependency> =
    when (artifactId) {
      "gezgin-core" ->
        setOf(
